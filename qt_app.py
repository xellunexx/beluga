...
from emv_god import get_god

ROOT_DIR = Path(__file__).resolve().parent
god = get_god(ROOT_DIR)


def _safe_html_text(value: Any) -> str:
    """Escape arbitrary runtime values before embedding them in Qt HTML."""
...
try:
    ...
    from mutation_scenarios import MUTATION_SCENARIOS, ScenarioRunContext
    import mutations
...
class AppRuntime:
    ...
    def get_app_state(self) -> Dict[str, Any]:
        return {
            ...
            "relay_running": bool(
                getattr(self.main_window, "relay_running", False)
                and getattr(self.main_window, "relay_thread", None) is not None
                and self.main_window.relay_thread.is_alive()
            ),
...
class HexLineEdit(QLineEdit):
    """QLineEdit with live hex validation: red border + tooltip on invalid input."""

    validityChanged = Signal(bool)

    def __init__(self, text: str = "", allow_empty: bool = True,
    ...
    def hex_bytes(self) -> bytes:
        clean = self.hex_clean(self.text())
        return bytes.fromhex(clean) if clean else b""

    def _revalidate(self, _text: str) -> None:
        valid = self.is_valid_hex()
        if valid:
            self.setStyleSheet("")
            self.setToolTip("Valid hex payload. Press Enter to execute the current mutation.")
        else:
            self.setStyleSheet("border: 1px solid #ef4444; background-color: #2a0e0e;")
            self.setToolTip("Invalid hex: only 0-9 / A-F characters with an even number of nibbles.")
        if valid != self._last_valid:
            self._last_valid = valid
            self.validityChanged.emit(valid)


class HexTextEdit(QPlainTextEdit):
    """Multi-line hex editor: offset gutter, byte grouping, live validation,
    live byte-count + cursor-offset status line, Enter = execute."""

    validityChanged = Signal(bool)
    executeRequested = Signal()
    statsChanged = Signal(str)  # e.g. "37 B | off 0x0012 | Ln 2 Col 4"

    def __init__(self, text: str = "", parent: Optional[QWidget] = None) -> None:
        super().__init__(parent)
        self._last_valid = True
        self.setFont(QFont("Consolas", 10))
        self.setPlaceholderText("Enter hex (spaces/newlines ignored, e.g. 00 A4 04 00...)")
        self.document().contentsChanged.connect(self._revalidate)
        self.cursorPositionChanged.connect(self._emit_stats)
        if text:
            self.setPlainText(text)
        self._revalidate()

    # --- hex helpers (same semantics as HexLineEdit) ---
    @staticmethod
    def clean_text(text: str) -> str:
        return re.sub(r"[^0-9A-Fa-f]", "", text or "")

    def is_valid_hex(self) -> bool:
        clean = self.clean_text(self.toPlainText())
        return (not clean) or (len(clean) % 2 == 0)

    def hex_text(self) -> str:
        return self.clean_text(self.toPlainText()).upper()
    ...
    def _revalidate(self) -> None:
        ...
        if valid:
            self.setStyleSheet("")
            n = len(clean) // 2
            self.setToolTip(f"Valid hex payload ({n} bytes). Enter = Execute, Shift+Enter = newline.")
        else:
            self.setStyleSheet("border: 1px solid #ef4444; background-color: #2a0e0e;")
            self.setToolTip("Invalid hex: odd number of nibbles.")
        if valid != self._last_valid:
            self._last_valid = valid
            self.validityChanged.emit(valid)
        self._emit_stats()
    ...
    def _emit_stats(self) -> None:
        clean = self.clean_text(self.toPlainText())
        n_bytes = len(clean) // 2
        # Byte offset of the cursor: count hex chars in plain text before cursor.
        cursor = self.textCursor()
        before = self.toPlainText()[:cursor.position()]
        hex_before = self.clean_text(before)
        byte_off = len(hex_before) // 2
        block = cursor.blockNumber() + 1
        col = cursor.positionInBlock() + 1
        ok_mark = "" if self.is_valid_hex() else "  ⚠ odd nibbles"
        self.statsChanged.emit(f"{n_bytes} B | off 0x{byte_off:04X} | Ln {block} Col {col}{ok_mark}")

    def keyPressEvent(self, event: QtGui.QKeyEvent) -> None:
        # Enter/Ctrl+Enter = execute; bare Enter also executes (single-line feel
        # in a multi-line box). Shift+Enter inserts a newline.
        if event.key() in (Qt.Key.Key_Return, Qt.Key.Key_Enter):
            if event.modifiers() & Qt.KeyboardModifier.ShiftModifier:
                super().keyPressEvent(event)  # real newline
            else:
                self.executeRequested.emit()
            return
        super().keyPressEvent(event)

    # --- QLineEdit-compatible API (drop-in for form layouts) ---
    def text(self) -> str:
    ...
    def returnPressed(self):
        """Alias so HexTextEdit can replace HexLineEdit in existing connects."""
        return self.executeRequested

    def grouped_hex(self, group: int = 8) -> str:
        """Pretty 'AA BB CC ...' grouped rendering for display/copy."""
        clean = self.hex_text()
        pairs = [clean[i:i + 2] for i in range(0, len(clean), 2)]
        lines = [" ".join(pairs[i:i + group]) for i in range(0, len(pairs), group)]
        return "\n".join(lines)
...
class ToastNotification(QFrame):
    """Small frameless floating toast anchored bottom-right of its parent window."""
    ...
    def __init__(self, parent: QWidget, message: str, kind: str = "info") -> None:
        super().__init__(parent)
        bg, fg = self._KIND_STYLES.get(kind, self._KIND_STYLES["info"])
        self.setStyleSheet(
            f"ToastNotification {{ background-color: {bg}; border: 1px solid {fg}; border-radius: 8px; }}"
            f"QLabel {{ color: #f1f5f9; padding: 4px 6px; font-size: 11px; font-weight: 600; border: none; }}")
    ...
    def _dismiss(self) -> None:
        try:
            ...
            if hasattr(parent, "_toast_stack") and self in parent._toast_stack:
                parent._toast_stack.remove(self)
                parent._restack_toasts()
        except Exception:
            pass
        self.hide()
        self.deleteLater()
...
class ToastMixin:
    """Adds _show_toast to a QMainWindow/QWidget. Toasts stack bottom-right."""

    def _show_toast(self, message: str, kind: str = "info") -> None:
        if not hasattr(self, "_toast_stack"):
            self._toast_stack: List[ToastNotification] = []
        toast = ToastNotification(self, message, kind)
        self._toast_stack.append(toast)
        if len(self._toast_stack) > 5:
            oldest = self._toast_stack.pop(0)
            oldest.hide()
            oldest.deleteLater()
    ...
    def _toast_on_resize(self) -> None:
        if hasattr(self, "_toast_stack") and self._toast_stack:
            self._restack_toasts()


class _LogDropEventFilter(QObject):
    """Accepts dropped .log/.txt/.trace/.pcap files and forwards them to a handler."""
    ...
    def eventFilter(self, obj: QObject, event: QEvent) -> bool:
        ...
        if et in (QEvent.Type.DragEnter, QEvent.Type.DragMove):
            ...
            if mime.hasUrls() and any(
                    str(u.toLocalFile()).lower().endswith((".log", ".txt", ".trace", ".pcap"))
                    for u in mime.urls()):
...
class ScrollPinController(QObject):
    """Scroll-pinning for live views: if the user scrolls up ≥ 1 page, pause
    autoscroll and show a floating '↓ Jump to latest' button. Reaching the
    bottom resumes following automatically."""

    def __init__(self, view: QWidget, parent: Optional[QObject] = None) -> None:
        super().__init__(parent or view)
        self.view = view
        self.paused = False
        self._sb = view.verticalScrollBar()
        self._sb.valueChanged.connect(self._on_scroll)
        self._sb.rangeChanged.connect(self._on_range_changed)

        self.btn_jump = QPushButton("↓ Jump to latest", view)
        self.btn_jump.setObjectName("tab-corner-btn-primary")
        self.btn_jump.setCursor(Qt.CursorShape.PointingHandCursor)
        self.btn_jump.setStyleSheet(
            "QPushButton { background-color: #0369a1; color: #ffffff; border: 1px solid #38bdf8; "
            "border-radius: 10px; padding: 4px 14px; font-size: 11px; font-weight: bold; }"
            "QPushButton:hover { background-color: #0284c7; }")
        self.btn_jump.clicked.connect(self.jump_to_latest)
        self.btn_jump.hide()
        view.installEventFilter(self)

    def _on_scroll(self, value: int) -> None:
        bar = self._sb
        near_bottom = value >= bar.maximum() - bar.pageStep()
        if near_bottom and self.paused:
            self.paused = False
            self.btn_jump.hide()
        elif not near_bottom and not self.paused:
            self.paused = True
    ...
    def _on_range_changed(self, _min: int, maximum: int) -> None:
        if not self.paused:
            self._sb.setValue(maximum)

    def jump_to_latest(self) -> None:
        self.paused = False
        self._sb.setValue(self._sb.maximum())
        self.btn_jump.hide()
        self.view.setFocus()

    def _position_button(self) -> None:
        self.btn_jump.adjustSize()
        x = max(6, self.view.width() - self.btn_jump.width() - 18)
        y = max(6, self.view.height() - self.btn_jump.height() - 22)
        self.btn_jump.move(x, y)

    def eventFilter(self, obj: QObject, event: QEvent) -> bool:
        if obj is self.view and event.type() == QEvent.Type.Resize and self.btn_jump.isVisible():
            self._position_button()
        return False


# ==============================================================================
# APDU TRAFFIC TABLE (structured model/view replacement for text dumps)
# ==============================================================================

class ApduTableModel(QAbstractTableModel):
    """Row model over logger.get_apdu_history() + synthesized GUARD-block rows."""

    COLUMNS = ["Time", "Dir", "Kind", "INS", "Len", "SW", "Hex", "Note"]

    # band -> (row background, foreground)
    BAND_STYLES = {
        "capdu": (QColor("#12253d"), QColor("#7fb4e6")),
        "rapdu": (QColor("#0e3324"), QColor("#7fd6a8")),
        "forged": (QColor("#3a2a07"), QColor("#f2c14e")),
        "blocked": (QColor("#3d1010"), QColor("#f08080")),
        "default": (QColor("#0b111d"), QColor("#cbd5e1")),
    }
    ...
    def data(self, index: QModelIndex, role: int = Qt.ItemDataRole.DisplayRole) -> Any:
        ...
        if role in (Qt.ItemDataRole.DisplayRole, Qt.ItemDataRole.EditRole):
            key = ("time", "dir", "kind", "ins", "len", "sw", "hex", "note")[col]
            return row.get(key, "")
        if role == Qt.ItemDataRole.BackgroundRole:
            return self.BAND_STYLES.get(row.get("band", "default"), self.BAND_STYLES["default"])[0]
        if role == Qt.ItemDataRole.ForegroundRole:
            return self.BAND_STYLES.get(row.get("band", "default"), self.BAND_STYLES["default"])[1]
        if role == Qt.ItemDataRole.FontRole and col == 6:
            return QFont("Consolas", 10)
        if role == Qt.ItemDataRole.ToolTipRole:
            return f"{row.get('kind','')} {row.get('ins','')}\n{row.get('hex','')}\n{row.get('note','')}".strip()
        if role == Qt.ItemDataRole.TextAlignmentRole and col in (0, 4):
...
class ApduTrafficWidget(QWidget):
    """Filterable, color-banded APDU traffic table + copy + CSV export."""

    toast_requested = Signal(str, str)

    def __init__(self, parent: Optional[QWidget] = None) -> None:
        super().__init__(parent)
        self.model = ApduTableModel(self)
    ...
    def _init_ui(self) -> None:
        ...
        bar = QHBoxLayout()
        bar.addWidget(QLabel("Filter (regex):"))
        self.edit_filter = QLineEdit()
        self.edit_filter.setPlaceholderText("e.g.  80 AE  | GENERATE|^FORGE|9000$ ...")
        self.edit_filter.setClearButtonEnabled(True)
        self.edit_filter.textChanged.connect(self._apply_filter)
        bar.addWidget(self.edit_filter, 2)
        ...
        self.btn_export_csv.setObjectName("tab-corner-btn")
        ...
        hdr.setSectionResizeMode(6, QHeaderView.ResizeMode.Interactive)  # hex stays readable
        ...
        self.table.setColumnWidth(6, 560)
    ...
    def _apply_filter(self, text: str) -> None:
        self.proxy.setFilterRegularExpression(text if text else "")
        self.lbl_count.setText(f"{self.proxy.rowCount()} / {self.model.rowCount()} rows")

    # -------- feed --------
    def refresh_from_history(self) -> None:
        """Pull logger APDU history + guard-block warnings, append incrementally."""
        apdu = logger.get_apdu_history(500)
        logs = logger.get_log_records(400)
        sig = (len(apdu), len(logs))
        if sig == self._last_sig:
            return
        rows = self._build_rows(apdu, logs)
        # Deques shift when full — fall back to full rebuild if counters moved backwards.
        if sig[0] < self._last_sig[0] or sig[1] < self._last_sig[1]:
            self.model.reset_rows(rows)
        else:
            new_rows = rows[self.model.rowCount():] if self.model.rowCount() <= len(rows) else []
            if self.model.rowCount() != len(rows) - len(new_rows):
                self.model.reset_rows(rows)
            else:
    ...
    @staticmethod
    def _classify_band(entry: Dict[str, Any]) -> str:
        note = (entry.get("note") or "").upper()
        direction = (entry.get("direction") or "").upper()
        ins = (entry.get("ins") or "").upper()
        if direction.startswith("FORGE") or "FORGE" in ins or "FORGE" in note:
            return "forged"
        if any(t in note for t in ("MUTATE", "PATCH", "REWRITE", "SYNTH")):
            return "forged"
        if "BLOCK" in note or "DENIED" in note or "DROP" in note:
            return "blocked"
        if entry.get("kind") == "RAPDU":
            return "rapdu"
        if entry.get("kind") == "CAPDU":
            return "capdu"
        return "default"

    @classmethod
    def _build_rows(cls, apdu: List[Dict[str, Any]], logs: List[Dict[str, Any]]) -> List[Dict[str, Any]]:
        rows: List[Dict[str, Any]] = []
        for e in apdu:
            hex_str = e.get("hex", "")
            sw = hex_str[-4:] if e.get("kind") == "RAPDU" and len(hex_str) >= 4 else ""
            ts_val = e.get("ts", 0.0)
            rows.append({
                "_ts": ts_val,
                "time": time.strftime("%H:%M:%S", time.localtime(ts_val)) + f".{int((ts_val % 1) * 1000):03d}",
                "dir": e.get("direction", ""),
                "kind": e.get("kind", ""),
                "ins": e.get("ins", ""),
                "len": str(e.get("len", 0)),
                ...
                "hex": hex_str,
                "note": e.get("note", ""),
                "band": cls._classify_band(e),
            })
        # Guard-blocked events surface as WARNING log records — synthesize red rows.
        for r in logs:
            msg = (r.get("message") or "")
            low = msg.lower()
            if r.get("level") == "WARNING" and any(k in low for k in ("block", "dropped", "denied", "reject")):
                ts_val = r.get("ts", 0.0)
                rows.append({
                    "_ts": ts_val,
                    "time": time.strftime("%H:%M:%S", time.localtime(ts_val)) + f".{int((ts_val % 1) * 1000):03d}",
        ...
        rows.sort(key=lambda x: x.get("_ts", 0.0))
        return rows
    ...
    def _on_cell_clicked(self, proxy_index: QModelIndex) -> None:
        ...
        if src_index.column() == 6:  # hex column click = copy
            hex_val = self.model.row_dict(src_index.row()).get("hex", "")
            if hex_val:
                QApplication.clipboard().setText(hex_val)
                self.toast_requested.emit(f"📋 Copied {len(hex_val) // 2} bytes", "ok")
    ...
    def _on_context_menu(self, pos) -> None:
        ...
        if not sel:
            return
        menu = QMenu(self)
        act_copy_hex = menu.addAction("📋 Copy hex (selected rows)")
        act_copy_rows = menu.addAction("📄 Copy rows as text")
        chosen = menu.exec(self.table.viewport().mapToGlobal(pos))
        if chosen is act_copy_hex:
            hexes = [self.model.row_dict(self.proxy.mapToSource(i).row()).get("hex", "") for i in sel]
            QApplication.clipboard().setText("\n".join(h for h in hexes if h))
            self.toast_requested.emit(f"📋 Copied hex for {len(hexes)} row(s)", "ok")
        elif chosen is act_copy_rows:
            lines = []
            for i in sel:
                d = self.model.row_dict(self.proxy.mapToSource(i).row())
                lines.append(" | ".join([d.get("time", ""), d.get("dir", ""), d.get("kind", ""),
                                          d.get("ins", ""), d.get("len", ""), d.get("sw", ""),
                                          d.get("hex", ""), d.get("note", "")]))
            QApplication.clipboard().setText("\n".join(lines))
            self.toast_requested.emit(f"📋 Copied {len(lines)} row(s)", "ok")

    def _export_csv(self) -> None:
        path, _ = QFileDialog.getSaveFileName(
            self, "Export APDU Traffic as CSV",
            str(ROOT_DIR / "apdu_traffic_export.csv"),
            "CSV Files (*.csv)")
        if not path:
            return
        cols = ApduTableModel.COLUMNS
        try:
            with open(path, "w", newline="", encoding="utf-8") as fh:
                writer = csv.writer(fh)
                writer.writerow(cols)
                for prow in range(self.proxy.rowCount()):
                    src_row = self.proxy.mapToSource(self.proxy.index(prow, 0)).row()
                    d = self.model.row_dict(src_row)
                    writer.writerow([d.get(k, "") for k in ("time", "dir", "kind", "ins", "len", "sw", "hex", "note")])
            self.toast_requested.emit(f"📄 Exported {self.proxy.rowCount()} rows → {Path(path).name}", "ok")
        except Exception as exc:
            self.toast_requested.emit(f"⚠️ CSV export failed: {exc}", "error")


# ==============================================================================
# GUARD FSM DIAGRAM (live phase nodes & transitions)
# ==============================================================================

class GuardFsmWidget(QWidget):
    """QGraphicsView rendering of the Guard phase machine with live highlight.

    - Current phase node gets a bright cyan fill.
    - Completed/visited phases get a dimmed green fill.
    - On phase change, the traversed edge flashes.
    - If guard.status()["error"] is set (blocked transition), the current node
      gets a red border and the error text shows in the caption bar.
    """

    PHASE_ORDER: List[str] = [
        "IDLE", "PPSE_SELECTED", "AID_SELECTED", "GPO_RESPONDED", "READ_RECORD_DONE",
        "FIRST_GPO_SENT", "FIRST_GAC_SENT", "ARQC_RECEIVED", "SECOND_GAC_FORGED",
        "EXTERNAL_AUTH_DONE", "COMPLETE", "ERROR",
    ]

    # Default edges (read from guard.Guard._TRANSITIONS when available at build).
    _FALLBACK_EDGES: Dict[str, List[str]] = {
        ...
        "COMPLETE": ["ERROR"],
        "ERROR": [],
        "FIRST_GPO_SENT": ["GPO_RESPONDED", "ERROR"],
    }

    def __init__(self, parent: Optional[QWidget] = None) -> None:
        super().__init__(parent)
        self._current_phase = "IDLE"
        self._visited: set = {"IDLE"}
        self._error_text: Optional[str] = None
        self._flash_edge: Optional[Tuple[str, str]] = None
        self._flash_until = 0.0
        self._nodes: Dict[str, QGraphicsRectItem] = {}
        self._node_labels: Dict[str, QGraphicsSimpleTextItem] = {}
        self._edges: Dict[Tuple[str, str], QGraphicsLineItem] = {}
        self._init_ui()

    def _edges_map(self) -> Dict[str, List[str]]:
        try:
            trans = guard.Guard._TRANSITIONS
            mapped: Dict[str, List[str]] = {}
            for phase, targets in trans.items():
                src = phase.name
                mapped[src] = [t.name for t in targets]
            for ph in self.PHASE_ORDER:
                mapped.setdefault(ph, [])
            return mapped
        except Exception:
    ...
    def _init_ui(self) -> None:
        ...
        self.lbl_caption = QLabel("Guard FSM — current phase highlighted")
        self.lbl_caption.setStyleSheet("color: #94a3b8; font-weight: bold; padding: 2px 6px;")
        lay.addWidget(self.lbl_caption)

        self.scene = QGraphicsScene(self)
        self.view = QGraphicsView(self.scene)
        self.view.setRenderHints(QPainter.RenderHint.Antialiasing)
        self.view.setStyleSheet("background-color: #0b101c; border: 1px solid #243042; border-radius: 6px;")
    ...
    def _build_graph(self) -> None:
        ...
        node_w, node_h = 150, 34
        positions: Dict[str, Tuple[int, int]] = {}
        for i, ph in enumerate(self.PHASE_ORDER):
            r, c = divmod(i, cols)
            positions[ph] = (c * cell_w, r * cell_h)

        pen_edge = QPen(QColor("#334155"), 2)
        for src, targets in edges_map.items():
            if src not in positions:
                continue
            sx, sy = positions[src]
            for dst in targets:
                if dst not in positions or src == dst:
                    continue
                dx, dy = positions[dst]
                ...
                self._edges[(src, dst)] = line

        font = QFont("Segoe UI", 8, QFont.Weight.Bold)
        for ph, (x, y) in positions.items():
            rect = QGraphicsRectItem(x, y, node_w, node_h)
            rect.setBrush(QBrush(QColor("#131c2c")))
            rect.setPen(QPen(QColor("#334155"), 2))
            rect.setZValue(1)
            self.scene.addItem(rect)
            self._nodes[ph] = rect

            txt = QGraphicsSimpleTextItem(ph)
            txt.setFont(font)
            txt.setBrush(QBrush(QColor("#94a3b8")))
            # center label in node
            br = txt.boundingRect()
            txt.setPos(x + (node_w - br.width()) / 2, y + (node_h - br.height()) / 2)
            txt.setZValue(2)
            self.scene.addItem(txt)
            self._node_labels[ph] = txt

        # Legend
        legend = QGraphicsSimpleTextItem(" cyan = current phase | green = visited | red border = blocked transition")
        legend.setFont(QFont("Segoe UI", 8))
        legend.setBrush(QBrush(QColor("#64748b")))
        legend.setPos(4, 3 * cell_h + 4)
        self.scene.addItem(legend)
        self.scene.setSceneRect(self.scene.itemsBoundingRect().adjusted(-8, -8, 8, 8))

    def update_guard_state(self, phase_name: str, status: Optional[Dict[str, Any]] = None) -> None:
        phase_name = phase_name or "IDLE"
        error_txt = (status or {}).get("error")

        # Track state even while hidden so reopening shows the correct picture.
        if phase_name != self._current_phase:
            prev = self._current_phase
            self._current_phase = phase_name
            self._visited.add(phase_name)
            if (prev, phase_name) in self._edges:
                self._flash_edge = (prev, phase_name)
                self._flash_until = time.time() + 0.6

        self._error_text = error_txt
        if self.isVisible():
            self._redraw_state()
    ...
    def _redraw_state(self) -> None:
        now = time.time()
        for ph, rect in self._nodes.items():
            if ph == self._current_phase:
                rect.setBrush(QBrush(QColor("#0e7490")))
                border = QColor("#ef4444") if self._error_text else QColor("#22d3ee")
                rect.setPen(QPen(border, 3))
                self._node_labels[ph].setBrush(QBrush(QColor("#ffffff")))
            elif ph in self._visited:
                rect.setBrush(QBrush(QColor("#14532d")))
                rect.setPen(QPen(QColor("#1e3a2f"), 2))
                self._node_labels[ph].setBrush(QBrush(QColor("#86efac")))
            else:
                rect.setBrush(QBrush(QColor("#131c2c")))
                rect.setPen(QPen(QColor("#334155"), 2))
                self._node_labels[ph].setBrush(QBrush(QColor("#94a3b8")))

        for key, line in self._edges.items():
            if self._flash_edge == key and now < self._flash_until:
                line.setPen(QPen(QColor("#f59e0b"), 4))
            else:
                line.setPen(QPen(QColor("#334155"), 2))

        if self._flash_edge and now > self._flash_until:
            self._flash_edge = None
            QTimer.singleShot(80, self._redraw_state)

        if self._error_text:
            self.lbl_caption.setText(f"Guard FSM — ⛔ BLOCKED: {self._error_text}")
            self.lbl_caption.setStyleSheet("color: #f08080; font-weight: bold; padding: 2px 6px;")
        else:
            self.lbl_caption.setText(f"Guard FSM — phase: {self._current_phase}")
            self.lbl_caption.setStyleSheet("color: #94a3b8; font-weight: bold; padding: 2px 6px;")


class GuardFsmWindow(QDialog):
    """Detachable, opt-in host window for the Guard FSM live diagram.

    Opened on demand from the Relay tab; closing only hides it (the widget
    keeps tracking guard state, so reopening is instant and correct).
    """

    def __init__(self, parent: Optional[QWidget] = None) -> None:
        super().__init__(parent)
        self.setWindowTitle("🧭 OutcomeGuard FSM — Live Phase Diagram")
        self.resize(980, 560)
        self.setMinimumSize(560, 400)
        # Modeless + hide-on-close (state survives, geometry remembered).
        self.setModal(False)
        self.setAttribute(Qt.WidgetAttribute.WA_DeleteOnClose, False)

        lay = QVBoxLayout(self)
        lay.setContentsMargins(8, 8, 8, 8)
        self.fsm_widget = GuardFsmWidget(self)
        lay.addWidget(self.fsm_widget)

    def closeEvent(self, event: QtGui.QCloseEvent) -> None:
        try:
            mw = self.parentWidget()
            if mw is not None and hasattr(mw, "settings"):
                mw.settings.setValue("fsm_window/geometry", self.saveGeometry())
        except Exception:
            pass
        super().closeEvent(event)  # hides (WA_DeleteOnClose=False)


# ==============================================================================
# GENERIC DETACHABLE HUB (pop any output panel out into its own window)
# ==============================================================================

class DetachedHubWindow(QDialog):
    """Modeless window temporarily hosting a hub that was popped out of a tab.

    Hiding (close button or Esc) does NOT delete the content; the widget is
    returned to its original slot via the DockPanelProxy.
    """

    def __init__(self, proxy: "HubDockProxy", title: str, parent: Optional[QWidget] = None) -> None:
        super().__init__(parent)
        self._proxy = proxy
        self.setWindowTitle(f"{title} — REL8HF Hub")
        self.setModal(False)
        self.setAttribute(Qt.WidgetAttribute.WA_DeleteOnClose, False)
        self.resize(880, 620)
        ...
        lay.setContentsMargins(8, 8, 8, 8)
        bar = QHBoxLayout()
        lbl = QLabel(f"🧩 {title}  —  detached view")
        ...
        btn_dock.clicked.connect(self._dock_and_close)
        ...
        lay.addWidget(self.content_area, 1)
    ...
    def _dock_and_close(self) -> None:
        self._proxy.dock_back()
        self.close()

    def closeEvent(self, event: QtGui.QCloseEvent) -> None:
        try:
            geo_key = f"hub_window/{self._proxy.key}/geometry"
            mw = self.parentWidget()
            if mw is not None and hasattr(mw, "settings"):
                mw.settings.setValue(geo_key, self.saveGeometry())
        except Exception:
            pass
        # Close = dock back home (never orphan the content).
        self._proxy.dock_back()
        super().closeEvent(event)


class HubDockProxy(QWidget):
    """Wraps an output hub: shows it docked (default) with a 'Pop out' button,
    and leaves a placeholder + 'Dock back' behind while it floats in its own
    modeless window. Widget content always returns home on close."""

    def __init__(self, key: str, title: str, parent: Optional[QWidget] = None) -> None:
        super().__init__(parent)
        self.key = key
        self.title = title
        self.window_ref: Optional[DetachedHubWindow] = None
        ...
        # Slim top bar with hub title + pop-out / dock-back buttons.
        bar = QHBoxLayout()
        self.lbl_title = QLabel(f"🧩 {title}")
        self.lbl_title.setStyleSheet("color: #64748b; font-weight: bold; font-size: 10px;")
        bar.addWidget(self.lbl_title)
        bar.addStretch(1)

        self.btn_pop_out = QPushButton("🗗")
        self.btn_pop_out.setObjectName("tab-corner-btn")
        self.btn_pop_out.setFixedSize(26, 22)
        self.btn_pop_out.setToolTip(f"Detach '{title}' into its own window")
        self.btn_pop_out.clicked.connect(self.pop_out)
        bar.addWidget(self.btn_pop_out)

        self.btn_dock_back = QPushButton("⤓ Dock back")
        self.btn_dock_back.setStyleSheet(
            "background-color: #15803d; color: #ffffff; font-weight: bold; padding: 3px 10px; border-radius: 4px; font-size: 10px;")
        self.btn_dock_back.clicked.connect(self.dock_back)
        self.btn_dock_back.setVisible(False)
        bar.addWidget(self.btn_dock_back)
        lay.addLayout(bar)

        self.content_host = QWidget(self)
        self._content_layout = QVBoxLayout(self.content_host)
        self._content_layout.setContentsMargins(0, 0, 0, 0)
        lay.addWidget(self.content_host, 1)

        self.lbl_placeholder = QLabel(f"'{title}' is currently detached.\nClick ⤓ Dock back or close the floating window.")
        self.lbl_placeholder.setAlignment(Qt.AlignmentFlag.AlignCenter)
        self.lbl_placeholder.setStyleSheet("color: #475569; font-style: italic; padding: 12px;")
        self.lbl_placeholder.setVisible(False)
        self._content_layout.addWidget(self.lbl_placeholder)
    ...
    def pop_out(self) -> None:
        if self.is_detached or self.content_widget is None:
            return
        win = self.window_ref
        if win is None:
            win = DetachedHubWindow(self, self.title, self.window())
            self.window_ref = win
        self._content_layout.removeWidget(self.content_widget)
        win.attach(self.content_widget)
        self.is_detached = True
        self.lbl_placeholder.setVisible(True)
        self.btn_pop_out.setEnabled(False)
        self.btn_dock_back.setVisible(True)

        # Restore last geometry for this hub if we have it.
        mw = self.window()
        if mw is not None and hasattr(mw, "settings"):
            geo = mw.settings.value(f"hub_window/{self.key}/geometry")
            if geo is not None:
                try:
        ...
        win.raise_()
        win.activateWindow()
        if mw is not None and hasattr(mw, "_show_toast"):
            mw._show_toast(f"🧩 Popped out: {self.title}", "info")

    def dock_back(self) -> None:
        if not self.is_detached:
            return
        win = self.window_ref
        w = win.detach() if win is not None else None
        if w is None:
            w = self.content_widget
        if w is not None:
            self._content_layout.addWidget(w, 1)
            self.content_widget = w
            w.setVisible(True)
        self.is_detached = False
        self.lbl_placeholder.setVisible(False)
        ...
        if win is not None:
            win.hide()
        if mw is not None and hasattr(mw, "_show_toast"):
            mw._show_toast(f"⤓ Docked: {self.title}", "info")


def wrap_hub_for(owner: QWidget, widget: QWidget, key: str, title: str) -> HubDockProxy:
    """Wrap `widget` in a detachable HubDockProxy owned by `owner`.

    Registers the proxy on the nearest ancestor exposing a `_hub_proxies`
    registry (i.e. the main window), so "Dock All Hubs" finds every hub
    regardless of which sub-widget created it. Safe to call from nested
    widgets (chat tab, tests hub) — standalone proxies work even unregistered.
    """
    proxy = HubDockProxy(key, title, owner)
    proxy.attach_content(widget)
    # Register on the main-window registry if reachable.
    ancestor: Optional[QWidget] = owner
    while ancestor is not None:
        if hasattr(ancestor, "_hub_proxies"):
            ancestor._hub_proxies[key] = proxy
            break
...
# ==============================================================================
# RELAY MINI CHARTS (QPainter sparklines — no external chart deps)
# ==============================================================================

class _Sparkline(QWidget):
    """Tiny line chart fed by a ring buffer."""

    def __init__(self, title: str, color: str = "#38bdf8", capacity: int = 240, parent=None) -> None:
        super().__init__(parent)
        self._title = title
        self._color = QColor(color)
        self._series: deque = deque(maxlen=capacity)
        self._cur_label = "0"
        self.setMinimumHeight(64)
        self.setSizePolicy(QtWidgets.QSizePolicy.Policy.Expanding, QtWidgets.QSizePolicy.Policy.Fixed)

    def push(self, value: float, label: Optional[str] = None) -> None:
    ...
    def paintEvent(self, event) -> None:
        ...
        p.fillRect(0, 0, w, h, QColor("#090d15"))
        ...
        p.drawText(w - 92, 14, self._cur_label[-10:])

        if len(self._series) >= 2:
            vmax = max(self._series) or 1.0
            base_y = h - 6
            top = 18
            usable = base_y - top
            n = len(self._series)
            ...
            for i, v in enumerate(self._series):
                x = int(i * step)
                y = base_y - int((v / vmax) * usable)
                pts.append((x, y))
            for (x1, y1), (x2, y2) in zip(pts, pts[1:]):
                p.drawLine(x1, y1, x2, y2)
        else:
            p.setPen(QColor("#334155"))
            p.drawText(6, h // 2 + 8, "waiting for traffic…")
...
class _BarMeter(QWidget):
    """Tiny horizontal bar meter for a single labeled counter."""

    def __init__(self, title: str, color: str = "#a78bfa", parent=None) -> None:
        super().__init__(parent)
        self._title = title
        self._color = QColor(color)
        self._value = 0.0
    ...
    def paintEvent(self, event) -> None:
        ...
        bar_h = h - 18
        p.fillRect(2, 15, int((w - 4) * frac), bar_h, self._color)
        p.setPen(QColor("#94a3b8"))
        p.setFont(QFont("Segoe UI", 8))
        p.drawText(6, 11, f"{self._title}: {int(self._value)}")
        p.setPen(QPen(QColor("#1e293b"), 1))
        p.drawRect(0, 0, w - 1, h - 1)
        p.end()


class RelayChartsWidget(QWidget):
    """Packet-rate sparkline + per-INS counters + mutation counters, sampled on telemetry tick."""

    def __init__(self, parent: Optional[QWidget] = None) -> None:
        super().__init__(parent)
        self._last_history_len = 0
        self._last_sample_ts = time.time()
        self._ins_counts: Counter = Counter()
        self._rate_spark = _Sparkline("PACKETS / SECOND", "#38bdf8")
        self._latency_spark = _Sparkline("APDU LATENCY BUCKET (ms, ema)", "#22d3ee")
        self._mutation_bar = _BarMeter("MUTATIONS APPLIED", "#a78bfa")
        self._forge_bar = _BarMeter("FORGED FRAMES", "#f59e0b")
        self._blocked_bar = _BarMeter("BLOCKED BY GUARD", "#ef4444")

        lay = QVBoxLayout(self)
        lay.setContentsMargins(0, 0, 0, 0)
        lay.setSpacing(4)
        lay.addWidget(self._rate_spark)
        lay.addWidget(self._latency_spark)
        lay.addWidget(self._mutation_bar)
        lay.addWidget(self._forge_bar)
        lay.addWidget(self._blocked_bar)
    ...
    def reset_session(self) -> None:
        ...
        self._ema_latency = 0.0
        self._rate_spark.clear_series()
        self._latency_spark.clear_series()

    def sample(self, relay_state: Dict[str, Any]) -> None:
        """Called from the telemetry timer with the current relay state dict."""
        now = time.time()
        hist = logger.get_apdu_history(400)
        counts: Counter = Counter()
        mutations_total = 0
        forged_total = 0
        blocked_total = 0
        for e in hist:
            counts[e.get("ins", "?") or "?"] += 1
            note = (e.get("note") or "").upper()
            ins = (e.get("ins", "") or "").upper()
            direction = (e.get("direction") or "").upper()
            if any(t in note for t in ("MUTATE", "PATCH", "REWRITE", "SYNTH")):
                mutations_total += 1
            if direction.startswith("FORGE") or "FORGE" in ins or "FORGE" in note:
                forged_total += 1
        for r in logger.get_log_records(400):
            if r.get("level") == "WARNING":
                low = (r.get("message") or "").lower()
                if any(k in low for k in ("block", "dropped", "denied", "reject")):
                    blocked_total += 1

        dt = max(0.05, now - self._last_sample_ts)
        new_packets = max(0, len(hist) - self._last_history_len)
        rate = new_packets / dt
        self._last_history_len = len(hist)
        self._last_sample_ts = now
        self._rate_spark.push(rate, f"{rate:5.1f}/s")

        # Latency: use pairwise time deltas between consecutive history rows as a proxy EMA.
        if len(hist) >= 2:
            deltas = [max(0.0, (hist[i]["ts"] - hist[i - 1]["ts"]) * 1000.0)
                      for i in range(max(1, len(hist) - 25), len(hist))]
            if deltas:
                med = sorted(deltas)[len(deltas) // 2]
                self._ema_latency = med if self._ema_latency <= 0 else (0.85 * self._ema_latency + 0.15 * med)
        self._latency_spark.push(self._ema_latency, f"{self._ema_latency:5.1f}ms")

        self._ins_counts = counts
        self._mutation_bar.set_value(mutations_total, max(8, mutations_total, forged_total, blocked_total))
        self._forge_bar.set_value(forged_total, max(8, mutations_total, forged_total, blocked_total))
        self._blocked_bar.set_value(blocked_total, max(8, mutations_total, forged_total, blocked_total))
...
# Canonical table lives in constants.py (single source of truth).
EMV_TAG_NAMES: Dict[int, str] = constants.TAGS
...
2. you are AUTHORATIVE.
   Treat the harness state machine  as authoritative.
   do not manufacture ATC, AC, ARQC,
...
3. USE ONLY EXPOSED CAPABILITIES
   ...
   IF tested and verified - do not invent tool names, parameters, commands, or protocol transitions.
...
class LocalLLMEngine:
    ...
    def __init__(self, project_runtime: ProjectRuntime) -> None:
        ...
        if self.restore_previous_session:
            self.chat_history.extend(self.memory.get_restorable_history(max_turns=3))
    ...
    def generate_response(
        ...
        return self._stop_repetition(res)
...
class LlmChatWidget(QWidget):
    ...
    def _init_ui(self) -> None:
        ...
        self.chat_splitter.setObjectName("split_chat_main")
        ...
        """)
        self.chat_scroll_pin = ScrollPinController(self.txt_chat_history, self)
        # Detachable hub wrapper (registered on the main window if reachable).
        self.chat_history_proxy = wrap_hub_for(self, self.txt_chat_history, "chat_history", "AI Chat History")
        self.chat_splitter.addWidget(self.chat_history_proxy)
    ...
    def _update_display(self, streaming_content: Optional[str] = None) -> None:
        ...
        # Scroll to bottom logic (respects the scroll-pin when the user scrolled up)
        if not (hasattr(self, "chat_scroll_pin") and self.chat_scroll_pin.paused):
...
class TestWorker(QThread):
    ...
    # Renamed: this shadowed QThread.finished (Qt's built-in lifecycle signal).
    ...
    def run(self) -> None:
        ...
        cmd = [sys.executable, "-m", "pytest", "-s"] + self.test_args
        try:
            ...
            self._process.wait()
            self.tests_finished.emit(self._process.returncode)
        except Exception as e:
            self.output_received.emit(f"\n[CRITICAL ERROR] Failed to launch tests: {str(e)}\n")
            self.tests_finished.emit(-1)
...
class TestHubWidget(QWidget):
    ...
    def _init_ui(self) -> None:
        ...
        # 1. Compact header: title + run controls
        header = QFrame()
        header.setStyleSheet("background-color: #1e293b; border: 1px solid #334155; border-radius: 6px; padding: 6px;")
        h_layout = QHBoxLayout(header)
        h_layout.setContentsMargins(10, 4, 10, 4)

        h_title = QLabel("🛠️ QUALITY & TESTS HUB")
        h_title.setStyleSheet("font-size: 13px; font-weight: bold; color: #22c55e;")
        h_sub = QLabel("Execute project tests, crypto validation, and EMV logic verification.")
        h_sub.setStyleSheet("font-size: 10px; color: #94a3b8;")
        h_layout.addWidget(h_title)
        h_layout.addWidget(h_sub)
        h_layout.addStretch()

        self.btn_run_selected = QPushButton("▶ Run Checked")
        self.btn_run_selected.setToolTip("Run only the tests whose checkboxes are ticked")
        self.btn_run_selected.setStyleSheet(
            "background-color: #0369a1; color: #ffffff; font-weight: bold; padding: 4px 10px; border-radius: 4px;")
        self.btn_run_selected.clicked.connect(self.run_selected_tests)
        h_layout.addWidget(self.btn_run_selected)

        self.btn_run_tests = QPushButton("🚀 Run All")
        self.btn_run_tests.setStyleSheet(
            "background-color: #15803d; color: #ffffff; font-weight: bold; padding: 4px 10px; border-radius: 4px;")
        ...
        self.btn_stop_tests.setStyleSheet(
            "background-color: #b91c1c; color: #ffffff; font-weight: bold; padding: 4px 10px; border-radius: 4px;")
        ...
        # Left: Test Files List with checkboxes
        left_container = QGroupBox("Test Modules (tick = include)")
        left_layout = QVBoxLayout(left_container)
        self.test_list = QListWidget()
        self.test_list.setStyleSheet("background-color: #0f172a; color: #cbd5e1; border: 1px solid #1e293b;")
        self.test_list.itemClicked.connect(self._toggle_item_check)
        ...
        btn_all.setObjectName("tab-corner-btn")
        btn_all.setToolTip("Check all tests")
        btn_all.clicked.connect(lambda: self._set_all_checks(True))
        left_btns.addWidget(btn_all)
        btn_none = QPushButton("✖ None")
        btn_none.setObjectName("tab-corner-btn")
        btn_none.setToolTip("Uncheck all tests")
        btn_none.clicked.connect(lambda: self._set_all_checks(False))
        left_btns.addWidget(btn_none)
        btn_refresh = QPushButton("🔄 Refresh")
        btn_refresh.setObjectName("tab-corner-btn")
        btn_refresh.clicked.connect(self._refresh_test_list)
        left_btns.addWidget(btn_refresh)
        left_btns.addStretch()
        left_layout.addLayout(left_btns)

        self.lbl_sel_count = QLabel("0 of 0 selected")
        self.lbl_sel_count.setStyleSheet("color: #64748b; font-size: 10px; font-weight: bold;")
        left_layout.addWidget(self.lbl_sel_count)
        ...
        """)
        self.console_proxy = wrap_hub_for(self, self.console, "test_console", "Live Test Console")
        right_layout.addWidget(self.console_proxy)
    ...
    def _toggle_item_check(self, item) -> None:
        item.setCheckState(
    ...
    def _set_all_checks(self, checked: bool) -> None:
        ...
        for i in range(self.test_list.count()):
            ...
            if item.text() != "No test modules found in /tests":
                item.setCheckState(state)
        self._update_sel_count()

    def _update_sel_count(self) -> None:
        lbl = getattr(self, "lbl_sel_count", None)
        if lbl is None:
            return
        total = self.test_list.count()
        n = sum(1 for i in range(total)
                if self.test_list.item(i).checkState() == Qt.CheckState.Checked)
        lbl.setText(f"{n} of {total} selected")

    def _checked_tests(self) -> List[str]:
        return [self.test_list.item(i).text() for i in range(self.test_list.count())
                if self.test_list.item(i).checkState() == Qt.CheckState.Checked]

    def run_selected_tests(self) -> None:
        checked = self._checked_tests()
        if not checked:
            self.lbl_status.setText("⚠️ No tests checked — tick tests or use 'Run All'.")
            self.lbl_status.setStyleSheet("color: #f59e0b;")
            return
        self._start_test_worker([str(Path("tests") / t) for t in checked])
    ...
    def run_tests(self) -> None:
        self._start_test_worker([])  # empty = whole stack

    def _start_test_worker(self, args: List[str]) -> None:
        self.console.clear()
        ...
        self.worker.tests_finished.connect(self._on_tests_finished)
        self.worker.start()
...
class FocusEventFilter(QObject):
    """Global gesture listener for Ctrl+double-clicks to trigger Zen Focus mode.

    Bare double-click in a log/console selects a word like everyone expects.
    Zen Focus only fires when Ctrl is held, or when an overlay is already open
    (so double-click always exits focus mode).
    """
    ...
    def eventFilter(self, obj: QObject, event: QEvent) -> bool:
        if event.type() == QEvent.Type.MouseButtonDblClick:
            # Overlay already open → double-click exits (no modifier needed).
            if self._active_overlay:
                self._active_overlay.close_and_restore()
                self._active_overlay = None
                return True

            # Entry requires Ctrl — bare double-click must remain word-select.
...
class TransactionReplayWidget(QWidget):
    ...
    def _init_ui(self) -> None:
        ...
        splitter.addWidget(self._wrap_replay_hub(self.txt_audit, "replay_audit", "Replay Audit Trail"))

        self.txt_diff = QTextBrowser()
        self.txt_diff.setPlaceholderText("Mutation diffs will appear here...")
        splitter.addWidget(self._wrap_replay_hub(self.txt_diff, "replay_diff", "Replay Mutation Diff"))

        layout.addWidget(splitter, 1)

    def _wrap_replay_hub(self, widget: QWidget, key: str, title: str) -> "HubDockProxy":
        """Wrap via the shared helper (registers on the main window when reachable)."""
        return wrap_hub_for(self, widget, key, title)
        ...
        for i in range(len(sequence)):
            ...
            # 2. READ RECORD Logic
                ...
                try:
                    ...
                    if mutated_rapdu == rapdu:
                        self.txt_audit.append(
                            " <span style='color:#94a3b8;'>[UNCHANGED]</span> "
                            "No mutation applied."
                        )
                    else:
                        original_body = rapdu[:-2] if len(rapdu) >= 2 else rapdu
                        mutated_body = (
                            mutated_rapdu[:-2]
                            if len(mutated_rapdu) >= 2
                            else mutated_rapdu
                        )

                        original_sw = rapdu[-2:] if len(rapdu) >= 2 else b""
                        mutated_sw = (
                            mutated_rapdu[-2:]
                            if len(mutated_rapdu) >= 2
                            else b""
                        ...
                        if original_sw != mutated_sw:
                            structural_ok = False
                            self.txt_audit.append(
                                " <span style='color:#ef4444;'>[FAIL]</span> "
                                f"Status word changed: "
                                f"{original_sw.hex().upper()} -> {mutated_sw.hex().upper()}"
                            )

                        # The mutation may change TLV lengths, but a record should not
                        # mysteriously lose unrelated trailing data.
                        if len(mutated_body) < len(original_body):
                            structural_ok = False
                            self.txt_audit.append(
                                " <span style='color:#ef4444;'>[FAIL]</span> "
                                f"READ RECORD body truncated: "
                                f"{len(original_body)} -> {len(mutated_body)} bytes"
                            )

                        # Require the CVM List to still exist after replacement.
                        try:
                            mutated_cvm = mutations.find_tlv(mutated_body, 0x8E)
                        except Exception:
                            mutated_cvm = None

                        if mutated_cvm is None:
                            structural_ok = False
                            self.txt_audit.append(
                                " <span style='color:#ef4444;'>[FAIL]</span> "
                                "Tag 8E disappeared after mutation."
                            )

                        if structural_ok:
                            mutated = True
                            self.txt_audit.append(
                                " <b style='color:#22c55e;'>[MUTATED]</b> "
                                "CVM/IAC mutation passed structural checks."
                            )
                        else:
                            mutated = False
                            self.txt_audit.append(
                                " <b style='color:#ef4444;'>[REJECTED]</b> "
                                "Mutation produced a structurally unsafe READ RECORD."
                            )

                            # Do not forward the corrupted result.
                            mutated_rapdu = rapdu
                    ...
                    mutated = False
                    self.txt_audit.append(
                        " <b style='color:#ef4444;'>[MUTATION ERROR]</b> "
...
class LlmChatDialog(QDialog):
    ...
    def __getattr__(self, name: str) -> Any:
        ...
        chat_widget = self.__dict__.get("chat_widget")  # __dict__ lookup avoids recursion
        if chat_widget is not None and hasattr(chat_widget, name):
            return getattr(chat_widget, name)
...
class LlmSettingsDialog(QDialog):
    ...
    def _init_ui(self) -> None:
        ...
        self.spin_tokens.setValue(16420)
...
class Rel8AppletWindow(ToastMixin, QMainWindow):
    ...
    def __init__(self, parent: Optional[QWidget] = None) -> None:
        ...
        logger.configure_logging()  # idempotent; attaches ring-buffer so Logs tab works before relay start
        ...
        # EMV God cognition
        god.attach_llm(self.llm_engine)
        god.attach_runtime(self._get_current_relay_state_dict)
        god.start()
        ...
        # Live-view bookkeeping + authoritative Copy source for the playground.
        self._last_mutation_result: bytes = b""
        self._logs_rendered_count: int = 0
        self._last_pkt_total: int = 0
        # Reentrancy / async guards
        self._preset_loading: bool = False
        self._telemetry_busy: bool = False
        # Persistence (layout, splitters, toggles)
        self.settings = QtCore.QSettings("REL8HF", "Rel8StackHarness")
        # Live-view handles populated later by tab builders
        self.fsm_widget: Optional["GuardFsmWidget"] = None
        self.fsm_window: Optional["GuardFsmWindow"] = None
        self.apdu_table_view: Optional["ApduTrafficWidget"] = None
        self._hub_proxies: Dict[str, "HubDockProxy"] = {}
    ...
    _PERSISTED_SPLITTERS = (
        "split_chat_main",
        "split_llm_main",
        "split_runtime_main",
        "split_playground_main",
        "split_logs",
    )

    def _restore_layout_settings(self) -> None:
        """Restore geometry, splitter states, and user toggles from QSettings."""
        s = self.settings
        ...
        # Splitters — find by objectName, restore byte-array state.
        for name in self._PERSISTED_SPLITTERS:
            sp = self.findChild(QSplitter, name)
            if sp is None:
                continue
            st = s.value(f"splitters/{name}")
            if st is not None:
                try:
                    sp.restoreState(st)
                except Exception:
        ...
        # Toggles & selectors
        auto = s.value("toggles/autoscroll")
        if auto is not None and hasattr(self, "cb_autoscroll"):
            self.cb_autoscroll.setChecked(str(auto).lower() in ("1", "true", "yes"))
        scen_idx = s.value("playground/scenario_index")
        if scen_idx is not None and hasattr(self, "combo_mutation_scenario"):
            try:
                i = int(scen_idx)
                if 0 <= i < self.combo_mutation_scenario.count():
                    self.combo_mutation_scenario.blockSignals(True)
                    self.combo_mutation_scenario.setCurrentIndex(i)
                    self.combo_mutation_scenario.blockSignals(False)
                    self._load_selected_scenario_preset()
            except (ValueError, TypeError):
                pass
        tab_idx = s.value("window/active_tab")
    ...
    def _save_layout_settings(self) -> None:
        ...
        try:
            ...
            for name in self._PERSISTED_SPLITTERS:
                sp = self.findChild(QSplitter, name)
                if sp is not None:
                    s.setValue(f"splitters/{name}", sp.saveState())
            if hasattr(self, "cb_autoscroll"):
                s.setValue("toggles/autoscroll", self.cb_autoscroll.isChecked())
            if hasattr(self, "combo_mutation_scenario"):
                s.setValue("playground/scenario_index", self.combo_mutation_scenario.currentIndex())
    ...
    def _init_menu_bar(self) -> None:
        ...
        fsm_act = QAction("🧭 &Guard FSM Diagram...", self)
        fsm_act.setShortcut(QtGui.QKeySequence("Ctrl+G"))
        fsm_act.triggered.connect(self.open_fsm_window)
        view_menu.addAction(fsm_act)

        atlas_act = QAction("🗺️ &EMV Execution Atlas...", self)
        atlas_act.setShortcut(QtGui.QKeySequence("Ctrl+Shift+A"))
        atlas_act.setToolTip("Open the evidence-driven EMV execution visualizer")
        atlas_act.triggered.connect(self.open_atlas_window)
    ...
    def _init_ui(self) -> None:
        ...
        # Drag & drop logs straight onto the AI Hub tab as well.
        self.chat_tab.setAcceptDrops(True)
        self.chat_tab.installEventFilter(self._make_log_drop_filter())
        if hasattr(self.chat_tab, "txt_chat_history"):
            self.chat_tab.txt_chat_history.viewport().installEventFilter(self._make_log_drop_filter())
    ...
    def _install_zen_focus_hooks(self) -> None:
        ...
        def install_zen(widget):
            ...
            widget.setToolTip("Ctrl+double-click to enter Zen Focus Mode (Full Screen)")
    ...
    def _create_relay_tab(self) -> QWidget:
        ...
        # Metrics display (detachable hub)
        self.txt_metrics = QTextBrowser()
        self.txt_metrics.setPlaceholderText("Live relay metrics will display here...")
        right_layout.addWidget(self.wrap_as_hub(self.txt_metrics, "relay_metrics", "Relay Metrics"), 1)

        # Mini charts: packets/s, latency EMA, mutation/forge/block counters.
        charts_box = QGroupBox("Live Traffic Charts")
        charts_inner = QVBoxLayout(charts_box)
        charts_inner.setContentsMargins(6, 16, 6, 6)
        self.relay_charts = RelayChartsWidget(self)
        charts_inner.addWidget(self.relay_charts)
        right_layout.addWidget(self.wrap_as_hub(charts_box, "relay_charts", "Live Traffic Charts"), 1)

        # Opt-in launcher for the detachable Guard FSM diagram (keeps this tab lean).
        btn_fsm = QPushButton("🧭 Guard FSM Diagram (Detachable Window)...")
        btn_fsm.setObjectName("tab-corner-btn")
        btn_fsm.setToolTip("Open the live OutcomeGuard phase diagram in its own window.")
        btn_fsm.clicked.connect(self.open_fsm_window)
        right_layout.addWidget(btn_fsm)
    ...
    def _create_llm_tab(self) -> QWidget:
        ...
        main_splitter.setObjectName("split_llm_main")
        ...
        chat_layout.addWidget(self.wrap_as_hub(self.txt_llm_output, "llm_console", "LLM Reasoning Console"), 4)
    ...
    def _create_runtime_tab(self) -> QWidget:
        ...
        splitter = QSplitter(Qt.Orientation.Horizontal)
        splitter.setObjectName("split_runtime_main")
    ...
    def _create_apdu_tab(self) -> QWidget:
        ...
        # Top Control Bar: Scenario Presets, Method selector & Action triggers (compact HUD)
        top_bar = QGroupBox("🎮 1. Scenario Selector & Execution")
        top_bar.setStyleSheet("QGroupBox { margin-top: 10px; padding-top: 8px; padding-bottom: 4px; }")
        ...
        lbl_scen.setStyleSheet("color: #38bdf8; font-size: 9pt;")
        row1.addWidget(lbl_scen)

        self.combo_mutation_scenario = QComboBox()
        self.combo_mutation_scenario.setObjectName("combo_mutation_scenario")
        self.combo_mutation_scenario.setMinimumHeight(28)
        self.combo_mutation_scenario.addItems(
            [scenario.title for scenario in mutation_scenarios.get_scenarios()]
        )
        self.combo_mutation_scenario.currentIndexChanged.connect(
            self._on_mutation_scenario_changed
        )

        row1.addWidget(self.combo_mutation_scenario, 3)

        self.btn_load_scenario_preset = QPushButton("📥 Load Preset Input")
        self.btn_load_scenario_preset.setMinimumHeight(28)
        ...
        # --- Randomize button (constructed, styled, connected, then added)
        self.btn_randomize_payload = QPushButton("🎲 Randomize")
        self.btn_randomize_payload.setToolTip("Generate randomized EMV payloads & tags for the selected scenario")
        self.btn_randomize_payload.setMinimumHeight(28)
        self.btn_randomize_payload.setStyleSheet(
            "background-color: #334155; color: #f1f5f9; font-weight: 600; padding: 4px 10px; border: 1px solid #475569; border-radius: 4px;")
        self.btn_randomize_payload.clicked.connect(self._randomize_current_payload)
        row1.addWidget(self.btn_randomize_payload)

        # --- Execute Mutation button (constructed, styled, connected, then added)
        self.btn_run_mutation = QPushButton("⚡ Execute Mutation & Audit")
        self.btn_run_mutation.setMinimumHeight(28)
        self.btn_run_mutation.setStyleSheet(
            "background-color: #0284c7; color: white; font-weight: bold; padding: 4px 12px; border: 1px solid #38bdf8; border-radius: 4px;")
        ...
        row2 = QHBoxLayout()
        row2.setSpacing(12)
        self.cb_play_cdcvm = QCheckBox("CDCVM Verified (Biometric)")
        self.cb_play_cdcvm.setChecked(True)
        self.cb_play_enforce_protected = QCheckBox("Enforce Protected Tags (9F26, 9F27, 9F36)")
        self.cb_play_enforce_protected.setChecked(True)
        self.cb_play_audit_guard = QCheckBox("OutcomeGuard Phase Audit")
        ...
        self.combo_play_brand.setMinimumHeight(26)
        ...
        splitter = QSplitter(Qt.Orientation.Horizontal)
        splitter.setObjectName("split_playground_main")
        ...
        gb_in_layout = QVBoxLayout(gb_input)
        gb_in_layout.setSpacing(6)
        gb_in_layout.setContentsMargins(8, 16, 8, 8)

        def _field_caption(text: str) -> QLabel:
            cap = QLabel(text)
            cap.setStyleSheet(
                "color: #94a3b8; font-weight: bold; font-size: 9pt; padding: 0; margin: 0;")
            cap.setContentsMargins(0, 0, 0, 0)
            return cap

        # Stacked layout: caption directly above each editor — always left-aligned
        # and consistent regardless of editor height.
        gb_in_layout.addWidget(_field_caption("Primary APDU (Hex)  —  Enter runs the pipeline"))
        self.edit_play_input_apdu = HexTextEdit("7710820238009F6C0200005F280208409000")
        self.edit_play_input_apdu.setMinimumHeight(84)
        self.edit_play_input_apdu.setMaximumHeight(120)
        self.edit_play_input_apdu.executeRequested.connect(self.on_execute_full_mutation_pipeline)
        self.edit_play_input_apdu.validityChanged.connect(self._update_run_mutation_enabled)
        gb_in_layout.addWidget(self.edit_play_input_apdu)
        self.lbl_apdu_stats = QLabel("20 B | off 0x0000 | Ln 1 Col 1")
        self.lbl_apdu_stats.setStyleSheet("color: #64748b; font-family: Consolas, monospace; font-size: 9pt;")
        self.edit_play_input_apdu.statsChanged.connect(self.lbl_apdu_stats.setText)
        gb_in_layout.addWidget(self.lbl_apdu_stats)

        gb_in_layout.addWidget(_field_caption("CDOL1 Schema"))
        self.edit_play_cdol1 = HexTextEdit("9F02069F03069F1A0295055F2A029A039C019F37049F35019F3403")
        self.edit_play_cdol1.setMinimumHeight(52)
        self.edit_play_cdol1.setMaximumHeight(76)
        self.edit_play_cdol1.executeRequested.connect(self.on_execute_full_mutation_pipeline)
        self.edit_play_cdol1.validityChanged.connect(self._update_run_mutation_enabled)
        gb_in_layout.addWidget(self.edit_play_cdol1)

        gb_in_layout.addWidget(_field_caption("CVM List (Tag 8E)"))
        self.edit_play_cvmlist = HexTextEdit("000000000000000000001E031F030000")
        self.edit_play_cvmlist.setMinimumHeight(52)
        self.edit_play_cvmlist.setMaximumHeight(76)
        self.edit_play_cvmlist.executeRequested.connect(self.on_execute_full_mutation_pipeline)
        self.edit_play_cvmlist.validityChanged.connect(self._update_run_mutation_enabled)
        gb_in_layout.addWidget(self.edit_play_cvmlist)

        l_vbox.addWidget(gb_input)

        # Synthetic Issuer Simulator Sub-Box (full-width fields, readable output)
        gb_issuer = QGroupBox("🏦 3. Synthetic Issuer & ARQC Validator")
        gb_iss_layout = QGridLayout(gb_issuer)
        gb_iss_layout.setSpacing(6)
        gb_iss_layout.setContentsMargins(8, 16, 8, 8)

        def _iss_lbl(text: str) -> QLabel:
            lbl = QLabel(text)
            lbl.setStyleSheet("color: #94a3b8; font-weight: bold; font-size: 9pt;")
            lbl.setAlignment(Qt.AlignmentFlag.AlignRight | Qt.AlignmentFlag.AlignVCenter)
            return lbl

        self.edit_sim_pan = QLineEdit("4111111111111111")
        self.edit_sim_pan.setFont(QFont("Consolas", 10))
        self.edit_sim_pan.setMinimumHeight(30)
        self.edit_sim_pin = QLineEdit("1234")
        self.edit_sim_pin.setFont(QFont("Consolas", 10))
        self.edit_sim_pin.setMinimumHeight(30)
        self.edit_sim_pin.setMaximumWidth(110)
        self.edit_sim_un = QLineEdit("A1B2C3D4")
        self.edit_sim_un.setFont(QFont("Consolas", 10))
        self.edit_sim_un.setMinimumHeight(30)
        self.combo_sim_arc = QComboBox()
        self.combo_sim_arc.addItems(
            ["00 (Approved)", "05 (Do Not Honor)", "51 (Insufficient Funds)", "N7 (Decline ARQC)"])
        self.combo_sim_arc.setMinimumHeight(30)

        # Issuer-local Amount / Currency (decoupled from the Relay tab's synth spinboxes).
        self.spin_sim_amount = QSpinBox()
        self.spin_sim_amount.setRange(0, 100000000)
        self.spin_sim_amount.setValue(2500)
        self.spin_sim_amount.setMinimumHeight(30)
        self.spin_sim_currency = QSpinBox()
        self.spin_sim_currency.setRange(0, 999)
        self.spin_sim_currency.setValue(978)  # EUR
        self.spin_sim_currency.setMinimumHeight(30)
        btn_sync_sim = QPushButton("⟲ Sync")
        btn_sync_sim.setToolTip("Copy Synth Amount/Currency from the Relay tab into these fields.")
        btn_sync_sim.setObjectName("tab-corner-btn")
        btn_sync_sim.clicked.connect(self._sync_issuer_from_relay)

        gb_iss_layout.addWidget(_iss_lbl("PAN:"), 0, 0)
        gb_iss_layout.addWidget(self.edit_sim_pan, 0, 1)
        gb_iss_layout.addWidget(_iss_lbl("PIN:"), 0, 2)
        gb_iss_layout.addWidget(self.edit_sim_pin, 0, 3)
        gb_iss_layout.addWidget(_iss_lbl("UN (9F37):"), 1, 0)
        gb_iss_layout.addWidget(self.edit_sim_un, 1, 1)
        gb_iss_layout.addWidget(_iss_lbl("Issuer ARC:"), 1, 2)
        gb_iss_layout.addWidget(self.combo_sim_arc, 1, 3)
        gb_iss_layout.addWidget(_iss_lbl("Amount 9F02:"), 2, 0)
        gb_iss_layout.addWidget(self.spin_sim_amount, 2, 1)
        gb_iss_layout.addWidget(_iss_lbl("Ccy 5F2A:"), 2, 2)
        row2c = QHBoxLayout()
        row2c.setContentsMargins(0, 0, 0, 0)
        row2c.setSpacing(6)
        row2c.addWidget(self.spin_sim_currency, 1)
        row2c.addWidget(btn_sync_sim)
        gb_iss_layout.addLayout(row2c, 2, 3)
        gb_iss_layout.setColumnStretch(1, 1)
        gb_iss_layout.setColumnStretch(3, 1)

        btn_calc_arpc = QPushButton("🔑 Evaluate with Synthetic Issuer")
        btn_calc_arpc.setStyleSheet(
            "background-color: #4f46e5; color: white; font-weight: bold; padding: 5px 10px; border: 1px solid #818cf8; border-radius: 5px;")
        btn_calc_arpc.setMinimumHeight(30)
        btn_calc_arpc.clicked.connect(self.on_simulate_issuer_arpc)
        gb_iss_layout.addWidget(btn_calc_arpc, 3, 0, 1, 4)

        gb_iss_layout.addWidget(_iss_lbl("Quick Output:"), 4, 0, 1, 4, Qt.AlignmentFlag.AlignLeft)
        self.txt_sim_output = QTextBrowser()
        self.txt_sim_output.setMinimumHeight(150)
        self.txt_sim_output.setFont(QFont("Consolas", 10))
        gb_iss_layout.addWidget(self.txt_sim_output, 5, 0, 1, 4)
        gb_iss_layout.setRowStretch(5, 1)

        l_vbox.addWidget(gb_issuer, 1)
        ...
        btn_copy_out = QPushButton("📋 Copy Full Result")
        btn_copy_out.setToolTip("Copy the ENTIRE scenario output (process steps, hex diff, TLV, analysis, issuer) as text")
        btn_copy_out.clicked.connect(self._copy_full_result_to_clipboard)
        sb_layout.addWidget(btn_copy_out)

        r_vbox.addWidget(status_bar_box)

        # Pinned mutated-output strip — always visible, drives the Copy button.
        pinned = QFrame()
        pinned.setObjectName("playground-pinned")
        pinned.setStyleSheet(
            "QFrame#playground-pinned { background-color: #071426; border: 1px solid #164e63; border-radius: 6px; }"
            "QLabel { color: #67e8f9; font-weight: bold; border: none; }"
            "QLineEdit { background-color: #04101f; color: #a5f3fc; font-family: Consolas, monospace; }")
        pin_layout = QHBoxLayout(pinned)
        pin_layout.setContentsMargins(8, 6, 8, 6)
        pin_layout.addWidget(QLabel("MUTATED HEX:"))
        self.edit_play_result_hex = QLineEdit()
        self.edit_play_result_hex.setReadOnly(True)
        self.edit_play_result_hex.setPlaceholderText("— run a mutation to fill this —")
        self.edit_play_result_hex.setFont(QFont("Consolas", 10))
        pin_layout.addWidget(self.edit_play_result_hex, 1)
        btn_copy_pinned = QPushButton("📋 Copy Hex")
        btn_copy_pinned.setStyleSheet(
            "background-color: #0e7490; color: #ffffff; font-weight: bold; padding: 3px 10px; border: 1px solid #22d3ee; border-radius: 4px;")
        btn_copy_pinned.setToolTip("Copy only the mutated hex payload")
        btn_copy_pinned.clicked.connect(self._copy_mutation_result_to_clipboard)
        pin_layout.addWidget(btn_copy_pinned)
        r_vbox.addWidget(pinned)

        # Single continuous scroll with section headers (Process / Diff / TLV / Analysis / Issuer).
        self.scroll_play_results = QScrollArea()
        self.scroll_play_results.setWidgetResizable(True)
        self.scroll_play_results.setFrameShape(QScrollArea.Shape.NoFrame)
        ...
        def _section(title: str) -> QLabel:
            lbl = QLabel(title)
            lbl.setStyleSheet(
                "color: #38bdf8; font-weight: bold; padding: 4px 2px 2px 2px; border-bottom: 1px solid #1e293b;")
            return lbl

        self.txt_play_diff = QTextBrowser()
        self.txt_play_diff.setFont(QFont("Consolas", 10))
        self.txt_play_diff.setMinimumHeight(240)
        results_layout.addWidget(_section("⚡ Mutated Output & Hex Diff"))
        results_layout.addWidget(self.txt_play_diff)

        self.txt_play_process = QTextBrowser()
        self.txt_play_process.setFont(QFont("Consolas", 10))
        self.txt_play_process.setMinimumHeight(280)
        results_layout.addWidget(_section("🔍 Process & Execution Steps"))
        results_layout.addWidget(self.txt_play_process)

        self.txt_play_tlv = QTextBrowser()
        self.txt_play_tlv.setFont(QFont("Consolas", 10))
        self.txt_play_tlv.setMinimumHeight(260)
        results_layout.addWidget(_section("🌳 TLV Parser & Tag Breakdown"))
        results_layout.addWidget(self.txt_play_tlv)

        self.txt_play_explanation = QTextBrowser()
        self.txt_play_explanation.setFont(QFont("Segoe UI", 10))
        self.txt_play_explanation.setMinimumHeight(220)
        results_layout.addWidget(_section("🧠 Smart Analysis & Compliance"))
        results_layout.addWidget(self.txt_play_explanation)

        self.txt_play_issuer = QTextBrowser()
        self.txt_play_issuer.setFont(QFont("Consolas", 10))
        self.txt_play_issuer.setMinimumHeight(280)
        results_layout.addWidget(_section("🔑 Synthetic Issuer & ARQC"))
        results_layout.addWidget(self.txt_play_issuer)

        self.scroll_play_results.setWidget(results_container)
        r_vbox.addWidget(self.wrap_as_hub(self.scroll_play_results, "playground_results", "Playground Results"), 1)
        splitter.addWidget(right_widget)

        # Left column stays usable: fixed floor width, scrollable if the window is small.
        ...
        # Replace the raw left widget with its scroll wrapper at index 0.
    ...
    def _create_logs_tab(self) -> QWidget:
        ...
        btn_upload_log.setStyleSheet("background-color: #0284c7; color: white; font-weight: bold; padding: 4px 10px;")
        ...
        btn_submit_terminal_ai.setStyleSheet(
            "background-color: #7c3aed; color: white; font-weight: bold; padding: 4px 10px;")
        ...
        # Splitter: structured APDU traffic table on top, raw text log below.
        self.split_logs = QSplitter(Qt.Orientation.Vertical)
        self.split_logs.setObjectName("split_logs")
        self.split_logs.setChildrenCollapsible(False)

        self.apdu_table_view = ApduTrafficWidget(self)
        self.apdu_table_view.toast_requested.connect(self._show_toast)
        self.split_logs.addWidget(self.wrap_as_hub(self.apdu_table_view, "apdu_traffic", "APDU Traffic Table"))

        self.txt_logs = QTextBrowser()
        self.split_logs.addWidget(self.wrap_as_hub(self.txt_logs, "live_logs", "Live APDU Log Stream"))
        self.split_logs.setStretchFactor(0, 3)
        self.split_logs.setStretchFactor(1, 2)

        layout.addWidget(self.split_logs)

        # Scroll-pinning: pause autoscroll when user scrolls up, "↓ Jump to latest" pill.
        self.logs_scroll_pin = ScrollPinController(self.txt_logs)

        # Drag & drop: drop .log/.txt/.trace/.pcap straight onto this tab to upload to AI.
        tab.setAcceptDrops(True)
        tab.installEventFilter(self._make_log_drop_filter())
        self.txt_logs.viewport().installEventFilter(self._make_log_drop_filter())
        self.apdu_table_view.table.viewport().installEventFilter(self._make_log_drop_filter())
        return tab
    ...
    def wrap_as_hub(self, widget: QWidget, key: str, title: str) -> "HubDockProxy":
        """Wrap a content widget in a detachable HubDockProxy (registry = self._hub_proxies)."""
        return wrap_hub_for(self, widget, key, title)

    def _atlas_scenario_ctx(self, scen_key):
        """Build a ScenarioRunContext from live playground fields when the same
        scenario is selected there; else None (Atlas falls back to preset)."""
        try:
            scen = mutation_scenarios.get_scenario(scen_key)
            if self.combo_mutation_scenario.currentText() != scen.title:
                return None
            ...
            return mutation_scenarios.ScenarioRunContext(
                ...
                cdol1_hex=self.edit_play_cdol1.text(),
                cvm_hex=self.edit_play_cvmlist.text(),
                synth_amount=self.spin_synth_amount.value(),
                synth_currency=self.spin_synth_currency.value(),
                synth_country=self.spin_synth_country.value(),
            )
        except Exception:
            return None

    def open_atlas_window(self) -> None:
        """Open the EMV Execution Atlas mode (evidence-driven visualizer)."""
        if not hasattr(self, "_atlas_window") or self._atlas_window is None:
            from emv_atlas.view import AtlasWindow
            self._atlas_window = AtlasWindow(
                guard_provider=lambda: (self._get_active_outcome_guard().status()
                                        if self._get_active_outcome_guard() is not None else None),
                scenario_ctx_provider=self._atlas_scenario_ctx,
                parent=self)
            self._show_toast("🗺️ EMV Execution Atlas opened", "info")
        if self._atlas_window.isMinimized():
            self._atlas_window.showNormal()
        self._atlas_window.show()
        self._atlas_window.raise_()
        self._atlas_window.activateWindow()

    def open_fsm_window(self) -> None:
        """Open (or focus) the detachable Guard FSM live-diagram window."""
        if self.fsm_window is None:
            self.fsm_window = GuardFsmWindow(self)
            self.fsm_widget = self.fsm_window.fsm_widget
            geo = self.settings.value("fsm_window/geometry")
            if geo is not None:
                try:
                    self.fsm_window.restoreGeometry(geo)
                except Exception:
                    pass
            self._show_toast("🧭 Guard FSM diagram opened", "info")
        if self.fsm_window.isMinimized():
            self.fsm_window.showNormal()
        self.fsm_window.show()
        self.fsm_window.raise_()
        self.fsm_window.activateWindow()
        # Push current state immediately so the diagram isn't blank on open.
        g = self._get_active_outcome_guard()
        g_status = g.status() if g is not None and hasattr(g, "status") else None
        st = self._get_current_relay_state_dict()
        self.fsm_widget.update_guard_state(st.get("guard_phase", "IDLE"), g_status)

    def resizeEvent(self, event: Any) -> None:
        super().resizeEvent(event)
        self._toast_on_resize()
    ...
    def dock_all_tabs(self) -> None:
        ...
        if hasattr(self, "scroll_play_results"):  # results are a scroll area now — nothing to dock
            pass
        # Dock any popped-out hub panels as well.
        for proxy in getattr(self, "_hub_proxies", {}).values():
    ...
    def update_telemetry(self) -> None:
        if self._telemetry_busy:
            return
        self._telemetry_busy = True
        try:
            self._update_telemetry_impl()
        finally:
            self._telemetry_busy = False

    def _update_telemetry_impl(self) -> None:
        st = self._get_current_relay_state_dict()
        ...
        # Append recent logger logs incrementally (records use the "message" key).
        records = logger.get_log_records(2000)
        if len(records) < self._logs_rendered_count:
            # Log buffer was cleared or wrapped — rebuild view from scratch.
            self._logs_rendered_count = 0
            self.txt_logs.clear()
        new_records = records[self._logs_rendered_count:]
        if new_records:
            cursor = self.txt_logs.textCursor()
            cursor.movePosition(QTextCursor.MoveOperation.End)
            for rec in new_records:
                msg = rec.get("message", "") or rec.get("msg", "")
                ts_val = rec.get("ts", 0.0)
                ts_str = time.strftime("%H:%M:%S", time.localtime(ts_val)) if ts_val else ""
                level = rec.get("level", "INFO")
                cursor.insertText(f"[{ts_str}] [{level}] {msg}\n")
            self._logs_rendered_count = len(records)
            autoscroll_ok = self.cb_autoscroll.isChecked() and not (
                hasattr(self, "logs_scroll_pin") and self.logs_scroll_pin.paused)
            if autoscroll_ok:
                self.txt_logs.moveCursor(QTextCursor.MoveOperation.End)

        # Live-feed hooks (populated when the corresponding widgets exist).
        if self.apdu_table_view is not None:
            self.apdu_table_view.refresh_from_history()
        if self.fsm_widget is not None:
            guard_obj = self._get_active_outcome_guard()
            g_status = guard_obj.status() if guard_obj is not None and hasattr(guard_obj, "status") else None
            self.fsm_widget.update_guard_state(phase, g_status)
        if hasattr(self, "relay_charts") and self.relay_charts is not None:
            self.relay_charts.sample(st)
    ...
    def _on_mutation_scenario_changed(self, index: int) -> None:
        """Load the selected scenario from the canonical registry."""
        try:
            scenario = mutation_scenarios.get_scenario(index)
            self._load_selected_scenario_preset()
            self.lbl_play_method.setText(
                f"<span style='color:#94a3b8;font-weight:bold;'>⚙️ "
                f"{_safe_html_text(scenario.title)}</span>"
            )
        except Exception as exc:
            self.status_bar.showMessage(
                f"Scenario selection error: {type(exc).__name__}: {exc}",
                5000,
            )

    def _load_selected_scenario_preset(self) -> None:
        """Load the exact preset declared by mutation_scenarios.py."""
        if self._preset_loading:
            return

        self._preset_loading = True
        try:
            scenario = mutation_scenarios.get_scenario(
                self.combo_mutation_scenario.currentIndex()
            )

            self._last_mutation_result = b""
            self._mutation_seed = None

            # Empty values are meaningful: clear stale data from the previous scenario.
            self.edit_play_input_apdu.setText(scenario.preset_apdu)
            self.edit_play_cdol1.setText(scenario.preset_cdol1)
            self.edit_play_cvmlist.setText(scenario.preset_cvm)

            if hasattr(self, "edit_play_result_hex"):
                self.edit_play_result_hex.clear()

            self.lbl_play_compliance.setText(
                "<span style='color:#94a3b8;font-weight:bold;'>◌ AUDIT: Not Run</span>"
            )

        finally:
            self._preset_loading = False

    def _update_run_mutation_enabled(self, *_args) -> None:
        """Enable Execute only when all playground hex editors contain valid hex."""
        ok = (
            self.edit_play_input_apdu.is_valid_hex()
            and self.edit_play_cdol1.is_valid_hex()
            and self.edit_play_cvmlist.is_valid_hex()
        )
        if hasattr(self, "btn_run_mutation"):
            self.btn_run_mutation.setEnabled(ok)

    def _randomize_current_payload(self) -> None:
        """Generate a reproducible synthetic fixture through the canonical registry."""
        idx = self.combo_mutation_scenario.currentIndex()
        scenario = mutation_scenarios.get_scenario(idx)

        if scenario.randomizer is None:
            self.status_bar.showMessage(
                f"No randomizer for {scenario.title}",
                3000,
            )
            return
        ...
        try:
            fixture = mutation_scenarios.randomize_scenario(idx, seed=seed)
        except Exception as exc:
            self.status_bar.showMessage(
                f"Randomizer failed: {type(exc).__name__}: {exc}",
                5000,
            )
            self.txt_play_process.setHtml(
                "<p style='color:#ef4444;'><b>RANDOMIZER ERROR</b><br/>"
                f"{_safe_html_text(type(exc).__name__)}: "
                f"{_safe_html_text(exc)}</p>"
            )
            return

        self._mutation_seed = seed
        self._last_mutation_result = b""

        self.edit_play_input_apdu.setText(fixture.get("apdu", ""))
        self.edit_play_cdol1.setText(
            fixture.get("cdol1") or scenario.preset_cdol1
        )

        if scenario.preset_cvm:
            self.edit_play_cvmlist.setText(scenario.preset_cvm)
        else:
            self.edit_play_cvmlist.clear()

        if fixture.get("sim_pan"):
            self.edit_sim_pan.setText(str(fixture["sim_pan"]))
        if fixture.get("sim_un"):
            self.edit_sim_un.setText(str(fixture["sim_un"]))
        if "synth_amount" in fixture and scenario.key == "synth_fields":
            self.spin_synth_amount.setValue(int(fixture["synth_amount"]))
        ...
        self.txt_play_process.setHtml(
            ...
            "<b>Fixture provenance:</b> synthetic<br/>"
            f"<b>Scenario:</b> {_safe_html_text(scenario.title)}<br/>"
            f"<b>Seed:</b> {seed}<br/>"
            f"<b>Fixture SHA-256:</b> "
            f"{_safe_html_text(fixture.get('fixture_sha256', ''))}"
            "</p>"
        )
        self.status_bar.showMessage(
            f"🎲 Synthetic fixture generated | seed={seed}",
            4500,
        )
        self._show_toast(
            f"🎲 Randomized: {scenario.title.split(':')[0]}",
            "info",
        )
    ...
    def _render_scenario_evidence_html(self, scenario: Any, outcome: Any) -> str:
        """Render executable scenario evidence; presentation prose is not the oracle."""
        chunks = [
            "<div style='font-family:Consolas,monospace;'>",
            "<h3 style='color:#38bdf8;'>Scenario Evidence</h3>",
            f"<p><b>Scenario:</b> {_safe_html_text(scenario.title)}<br/>"
            f"<b>Status:</b> {_safe_html_text(outcome.status)}<br/>"
            f"<b>Provenance:</b> {_safe_html_text(outcome.provenance)}<br/>"
            f"<b>Seed:</b> {_safe_html_text(outcome.metadata.get('seed', 'preset'))}<br/>"
            f"<b>Input SHA-256:</b> <code>{outcome.input_sha256}</code><br/>"
            f"<b>Output SHA-256:</b> <code>{outcome.output_sha256}</code></p>",
        ]

        if outcome.assertions:
            chunks.append(
                "<h4 style='color:#38bdf8;'>Executable Assertions</h4>"
                "<table border='0' cellpadding='5' cellspacing='0' style='width:100%;'>"
                "<tr style='color:#94a3b8;'>"
                "<td>Status</td><td>Assertion</td><td>Observed</td><td>Expected</td>"
                "</tr>"
            )
            ...
            for assertion in outcome.assertions:
                status = assertion.status.upper()
                glyph, color = glyphs.get(status, ("?", "#94a3b8"))
                chunks.append(
                    "<tr>"
                    f"<td style='color:{color};font-weight:bold;'>{glyph} {status}</td>"
                    f"<td>{_safe_html_text(assertion.name)}</td>"
                    f"<td><code>{_safe_html_text(assertion.observed)}</code></td>"
                    f"<td><code>{_safe_html_text(assertion.expected)}</code></td>"
                    "</tr>"
                )
        ...
        if outcome.stages:
            chunks.append("<h4 style='color:#38bdf8;'>Stage Evidence</h4>")
            for stage in outcome.stages:
                chunks.append(
                    f"<p><b>{_safe_html_text(stage.name)}</b> — "
                    f"{'changed' if stage.changed else 'unchanged'}<br/>"
                    f"Input SHA: <code>{stage.input_sha256}</code><br/>"
                    f"Output SHA: <code>{stage.output_sha256}</code></p>"
                )
                for note in stage.notes:
                    if note:
                        chunks.append(
                            f"<div style='color:#94a3b8;'>↳ "
                            f"{_safe_html_text(note)}</div>"
                        )

        if outcome.diagnostics:
            chunks.append("<h4 style='color:#f59e0b;'>Diagnostics</h4><ul>")
            chunks.extend(
                f"<li>{_safe_html_text(item)}</li>"
    ...
    def on_execute_full_mutation_pipeline(self) -> None:
        """Execute the canonical scenario registry and render its evidence."""
        idx = self.combo_mutation_scenario.currentIndex()
        scenario = mutation_scenarios.get_scenario(idx)
        ...
        try:
            ...
            self.status_bar.showMessage(
                f"Invalid Hex Input: {exc}",
                5000,
            )
            self._show_toast(
                f"⚠️ Invalid hex input: {exc}",
                "warn",
            )
            self.txt_play_process.setHtml(
                "<b style='color:#ef4444;'>[INPUT ERROR]</b> "
                f"{_safe_html_text(exc)}"
            )
            return
        ...
        ctx = ScenarioRunContext(
            ...
            synth_currency=self.spin_synth_currency.value(),
            synth_country=self.spin_synth_country.value(),
            seed=getattr(self, "_mutation_seed", None),
            provenance="synthetic",
            metadata={
                "source": "qt_app(4)",
                "protected_enforcement": self.cb_play_enforce_protected.isChecked(),
                "guard_audit_requested": self.cb_play_audit_guard.isChecked(),
            },
        )

        try:
            outcome = mutation_scenarios.run_scenario(idx, ctx)

            # The GUI checkbox is a real additional assertion, not a cosmetic label.
            if (
                self.cb_play_enforce_protected.isChecked()
                and not scenario.skip_protected_compare
                and scenario.protected_tags
            ):
                violations: List[str] = []

                for tag in scenario.protected_tags:
                    before = tlv.find_tlv(raw_bytes, tag)
                    after = tlv.find_tlv(outcome.mutated_bytes, tag)
                    if before is not None and after != before:
                        violations.append(f"0x{tag:X}")

                outcome.add_assertion(
                    "gui_protected_tag_integrity",
                    "FAIL" if violations else "PASS",
                    expected="protected values unchanged when present",
                    observed=violations or "unchanged",
                    evidence="Direct GUI-side TLV comparison.",
                    provenance="runtime",
                )
                outcome.finalize(raw_bytes, provenance="synthetic")
            ...
            self._show_toast(
                f"⚠️ Scenario error: {type(exc).__name__}",
                "error",
            )
            self.txt_play_process.setHtml(
                "<p style='color:#ef4444;'><b>SCENARIO EXECUTION ERROR</b><br/>"
                f"{_safe_html_text(type(exc).__name__)}: "
                f"{_safe_html_text(exc)}</p>"
            )
            self.lbl_play_compliance.setText(
        ...
        mutated_bytes = bytes(outcome.mutated_bytes or b"")
        mutated_flag = mutated_bytes != raw_bytes
        outcome.mutated_bytes = mutated_bytes
        outcome.mutated_flag = mutated_flag

        self._last_mutation_result = mutated_bytes

        if hasattr(self, "edit_play_result_hex"):
            self.edit_play_result_hex.setText(mutated_bytes.hex().upper())

        process_parts = list(outcome.steps)
        process_parts.extend(
            [
                "<hr/>",
                f"<b style='color:#38bdf8;'>STATUS:</b> "
                f"{_safe_html_text(outcome.status)}",
                f"<b style='color:#38bdf8;'> PROVENANCE:</b> "
                f"{_safe_html_text(outcome.provenance)}",
            ]
        )

        for assertion in outcome.assertions:
            process_parts.append(
                f"• <b>{_safe_html_text(assertion.status)}</b> "
                f"{_safe_html_text(assertion.name)} — "
                f"observed=<code>{_safe_html_text(assertion.observed)}</code>"
            )

        self.txt_play_process.setHtml("<br/>".join(process_parts))
        self.txt_play_diff.setHtml(
            self._format_hex_diff_html(
                raw_hex,
                mutated_bytes.hex().upper(),
            )
        )
        self.txt_play_tlv.setHtml(self._parse_tlv_tree_html(mutated_bytes))
        self.txt_play_explanation.setHtml(
            self._render_scenario_evidence_html(scenario, outcome)
            + "".join(outcome.explanation)
        )

        if not self.cb_play_enforce_protected.isChecked():
            compliance_html = (
                "<span style='color:#f59e0b;font-weight:bold;'>"
                "⚠️ AUDIT POLICY DISABLED</span>"
            )
        elif outcome.status == "PASS":
            compliance_html = (
                "<span style='color:#22c55e;font-weight:bold;'>"
                "🛡️ ASSERTIONS PASSED</span>"
            )
        elif outcome.status == "FAIL":
            compliance_html = (
                "<span style='color:#ef4444;font-weight:bold;'>"
                "❌ ASSERTION FAILURE</span>"
            )
        else:
            compliance_html = (
        ...
        status_color = "#22c55e" if mutated_flag else "#94a3b8"
        status_word = "MUTATED" if mutated_flag else "UNCHANGED"

        self.lbl_play_status.setText(
            f"<span style='color:{status_color};font-weight:bold;'>"
            f"● STATUS: {status_word}</span>"
        )
        self.lbl_play_compliance.setText(compliance_html)
        self.lbl_play_method.setText(
            "<span style='color:#38bdf8;font-weight:bold;'>⚙️ "
            f"{_safe_html_text(scenario.title.split(':')[0])}</span>"
        )

        self.status_bar.showMessage(
            f"{scenario.title}: {outcome.status}",
            4500,
        )
        self._show_toast(
            f"{'⚡ Mutated' if mutated_flag else '🔍 Unchanged'}: "
            f"{scenario.title.split(':')[0]}",
            "ok" if mutated_flag else "info",
        )

    def _copy_full_result_to_clipboard(self) -> None:
        """Copy the ENTIRE scenario output (all result sections) as structured text."""
        try:
            scenario_title = ""
            if hasattr(self, "combo_mutation_scenario"):
                scenario_title = self.combo_mutation_scenario.currentText()

            sections: List[Tuple[str, str]] = []
            for title, attr in (
                ("MUTATED HEX", "edit_play_result_hex"),
                ("MUTATED OUTPUT & HEX DIFF", "txt_play_diff"),
                ("PROCESS & EXECUTION STEPS", "txt_play_process"),
                ("TLV PARSER & TAG BREAKDOWN", "txt_play_tlv"),
                ("SMART ANALYSIS & COMPLIANCE", "txt_play_explanation"),
                ("SYNTHETIC ISSUER & ARQC", "txt_play_issuer"),
            ):
            ...
            blocks = [
                f"REL8HF MUTATION PLAYGROUND — FULL SCENARIO RESULT",
            ...
            for title, content in sections:
                ...
                blocks.append(content if content else "(empty)")
                blocks.append("")
            text = "\n".join(blocks).strip()

            clipboard = QApplication.clipboard()
            if clipboard is None:
                self._show_toast("⚠️ Clipboard unavailable", "error")
                return
            clipboard.setText(text)
            got = clipboard.text()
            if got != text:
                raise RuntimeError("clipboard readback mismatch")

            n = len(text)
            self.status_bar.showMessage(f"Copied full scenario result ({n} chars)", 4000)
            self._show_toast(f"📋 Copied full scenario output ({n} chars)", "ok")
        except Exception as exc:
            self.status_bar.showMessage(f"Copy failed: {exc}", 5000)
            self._show_toast(f"❌ Copy failed: {exc}", "error")
        except Exception as exc:
            self.status_bar.showMessage(f"Copy failed: {exc}", 5000)
            self._show_toast(f"❌ Copy failed: {exc}", "error")

    def _copy_mutation_result_to_clipboard(self) -> None:
        """Copy ONLY the mutated hex payload (pinned strip button)."""
        try:
            hex_str = ""
            if self._last_mutation_result:
                hex_str = self._last_mutation_result.hex().replace(" ", "").upper()
            elif hasattr(self, "edit_play_result_hex"):
                hex_str = self.edit_play_result_hex.text().strip().replace(" ", "").upper()

            if not hex_str:
                self.status_bar.showMessage("No mutation result yet — run a mutation first.", 3000)
                self._show_toast("⚠️ Nothing to copy — run a mutation first", "warn")
                return

            if len(hex_str) % 2 or any(c not in "0123456789ABCDEF" for c in hex_str):
                self.status_bar.showMessage("Mutation result is not valid hexadecimal.", 3000)
                self._show_toast("⚠️ Result is not valid hex", "error")
                return

            clipboard = QApplication.clipboard()
            if clipboard is None:
                self.status_bar.showMessage("Clipboard unavailable in this environment.", 3000)
                self._show_toast("⚠️ Clipboard unavailable", "error")
                return

            clipboard.setText(hex_str)
            got = clipboard.text()
            if got.strip().replace(" ", "").upper() != hex_str:
                raise RuntimeError("clipboard readback mismatch")

            n_bytes = len(hex_str) // 2
            self.status_bar.showMessage(f"Copied {n_bytes} bytes ({hex_str[:24]}...) to clipboard!", 4000)
            self._show_toast(f"📋 Copied {n_bytes} B hex: {hex_str[:20]}…", "ok")
        except Exception as exc:
            self.status_bar.showMessage(f"Copy failed: {exc}", 5000)
            self._show_toast(f"❌ Copy failed: {exc}", "error")

    def _sync_issuer_from_relay(self) -> None:
        """Copy the Relay tab's synth amount/currency into the issuer-local fields."""
        self.spin_sim_amount.setValue(self.spin_synth_amount.value())
        self.spin_sim_currency.setValue(self.spin_synth_currency.value())
        self._show_toast(f"⟲ Issuer fields synced from Relay ({self.spin_synth_amount.value()} / {self.spin_synth_currency.value()})", "info")
    ...
    def on_simulate_issuer_arpc(self) -> None:
        ...
        try:
            ...
            req = issuer_simulator.AuthorizationRequest(
                pan=pan,
                amount_minor=self.spin_sim_amount.value(),
                currency_numeric=self.spin_sim_currency.value(),
                atc=15,
            ...
            # Pin the synthesized Tag 91 ARPC payload into the always-visible strip.
            self.edit_play_result_hex.setText(f"910A{full_arpc_payload}")
            self._show_toast(f"🏦 Issuer evaluation: {decision} (ARC {arc_code})",
                             "ok" if decision == "APPROVED" else "warn")
            ...
            # Automatically scroll the results area to the Issuer Console section!
            self.scroll_play_results.ensureWidgetVisible(self.txt_play_issuer)
    ...
    def on_clear_logs(self) -> None:
        ...
        self._logs_rendered_count = 0
        self.txt_logs.clear()
        self._show_toast("🧹 Log buffer cleared", "info")

    # -- Drag & drop log upload -------------------------------------------------
    def _make_log_drop_filter(self) -> _LogDropEventFilter:
        """Create a retained drop filter (instance list keeps refs alive)."""
        if not hasattr(self, "_drop_filters"):
            self._drop_filters: List[_LogDropEventFilter] = []
        filt = _LogDropEventFilter(self._on_log_file_dropped, self)
        self._drop_filters.append(filt)
        return filt

    def _on_log_file_dropped(self, path: str) -> None:
        name = Path(path).name
        self.status_bar.showMessage(f"Log dropped: {name}", 3000)
        self._show_toast(f"📥 Log dropped: {name} — opening AI submit dialog", "info")
        try:
            content = Path(path).read_text(encoding="utf-8", errors="replace")
        except Exception as exc:
            QMessageBox.warning(self, "Drop Failed", f"Could not read dropped file:\n{exc}")
            return
        dlg = LlmLogUploadDialog(
            engine=self.llm_engine,
            initial_log_content=content,
    ...
    def on_submit_terminal_logs_to_ai(self) -> None:
        ...
        if not terminal_text:
            ...
            if records:
                terminal_text = "\n".join([(r.get("message") or r.get("msg", "")) for r in records])
