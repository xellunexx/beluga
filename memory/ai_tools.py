...
from emv_cognition import EMVCognitionEngine

try:
    from emv_god import EMVProjectIntelligence
except Exception:
    EMVProjectIntelligence = None
...
class AIToolRegistry:
    ...
    def __init__(
        ...
        self.project_intelligence: Optional[Any] = None

        root_dir = Path(__file__).resolve().parent

        # The old EMV Guardian layer was removed. Do not recreate or reference it.
        # Keep the surviving cognition/God layers explicit and observable.
        logger.warning("AI_BOOT: cognition.begin")
        try:
            self.emv_cognition = EMVCognitionEngine(
                root_dir / "memory" / "emv_cognition.json"
            )
            logger.warning("AI_BOOT: cognition.ok")
        except Exception:
            logger.exception("AI_BOOT: EMVCognitionEngine initialization failed")
            self.emv_cognition = None

        logger.warning("AI_BOOT: god_core.begin")
        try:
            self.god_core = emv_god.EMVGodCore(root_dir)
            logger.warning("AI_BOOT: god_core.ok")
        except Exception:
            logger.exception("AI_BOOT: EMVGodCore initialization failed")
            self.god_core = None

        logger.warning("AI_BOOT: register_tools.begin")
        self._register_default_tools()
        logger.warning(
            "AI_BOOT: register_tools.ok count=%d",
            len(self.tools),
        )

        logger.warning("AI_BOOT: layer2.begin")
        self._init_emv_layer2()
        logger.warning(
            "AI_BOOT: registry.complete tools=%d",
            len(self.tools),
        )
    ...
    def runtime_health(self) -> Dict[str, Any]:
        """Return a non-blocking snapshot of AI infrastructure state."""
        return {
            "status": "ok",
            "tools": len(self.tools),
            "emv_cognition": self.emv_cognition is not None,
            "god_core": getattr(self, "god_core", None) is not None,
            "project_runtime": self.project_runtime is not None,
            "app_runtime_getter": self.app_runtime_getter is not None,
            "relay_state_getter": self.relay_state_getter is not None,
            "outcome_guard_getter": self.outcome_guard_getter is not None,
    ...
    def _init_emv_layer2(self) -> None:
        """Attach persistent project intelligence without breaking startup."""
        if EMVProjectIntelligence is None:
            return
        try:
            root = Path(__file__).parent.resolve()
            self.project_intelligence = EMVProjectIntelligence(root)
            self.register(AITool(
                name="god_project_map",
                description="Refresh and inspect the live project architecture, symbols, imports, dependents, plans, recent changes, verification history, and lessons.",
                parameters={"type": "object", "properties": {}, "additionalProperties": False},
                handler=lambda: self.project_intelligence.project_map(),
                category="god",
            ))
            self.register(AITool(
                name="god_verify_change",
                description="Verify an actual file on disk after a modification. For Python, parse it and return the current hash and errors.",
                parameters={
                    "type": "object",
                ...
                handler=lambda file_path: self.project_intelligence.verify_file(file_path),
                category="god",
            ))
            self.register(AITool(
                name="god_set_plan",
                description="Create a persistent engineering plan with explicit steps that can be updated as work proceeds.",
                parameters={
                    "type": "object",
                    "properties": {
                        "task": {"type": "string"},
                ...
                handler=lambda task, steps: self.project_intelligence.set_plan(task, steps),
                category="god",
            ))
            self.register(AITool(
                name="god_plan_step",
                description="Mark one persistent engineering-plan step pending, active, done, or failed.",
                parameters={
                    "type": "object",
                    "properties": {
                        "task": {"type": "string"},
                    ...
                    "required": ["task", "index", "status"],
                    "additionalProperties": False,
                },
                handler=lambda task, index, status: self.project_intelligence.update_plan_step(task, index, status),
                category="god",
            ))
            self.register(AITool(
                name="god_record_lesson",
                description="Persist a reusable engineering lesson learned from an actual observed result.",
                parameters={
                    "type": "object",
                    "properties": {
                        "lesson": {"type": "string"},
                ...
                handler=lambda lesson, evidence=None: self.project_intelligence.add_lesson(lesson, evidence),
                category="god",
            ))
        except Exception:
            logger.exception("EMV Layer 2 initialization failed; continuing normally.")

    def get_emv_layer2_context(self, max_chars: int = 12000) -> str:
        try:
            if self.project_intelligence:
                return self.project_intelligence.context(max_chars=max_chars)
        except Exception:
            logger.exception("Failed to build EMV Layer 2 context.")
        return ""


    def get_emv_layer3_context(self, max_chars: int = 7000) -> str:
        """Return higher-level project context using the surviving EMV God core."""
        try:
            god_core = getattr(self, "god_core", None)
            if god_core and hasattr(god_core, "context"):
                max_symbols = max(24, min(120, max_chars // 140))
                return god_core.context(max_symbols=max_symbols)
        except Exception:
            logger.exception("Failed to build EMV higher-level context")
        return ""

    def _tool_god_project_map(self, refresh: bool = False) -> Dict[str, Any]:
        """Return the living project map used by the EMV God cognition layer."""
        if not self.god_core:
            return {"success": False, "error": "EMV God core unavailable"}
        if refresh or not self.god_core.state.get("files"):
            indexed = self.god_core.index_project()
        else:
            indexed = {"files": len(self.god_core.state.get("files", {})), "symbols": sum(len(v) for v in self.god_core.state.get("symbols", {}).values())}
        return {"success": True, "index": indexed, "context": self.god_core.context()}

    def _tool_god_verify(self, file_path: str, expected_sha256: str = "") -> Dict[str, Any]:
        """Re-read and verify a project file after an agent mutation."""
        if not self.god_core:
            return {"success": False, "error": "EMV God core unavailable"}
        return self.god_core.verify_file(file_path, expected_sha256)

    def _tool_god_set_task(self, task: str) -> Dict[str, Any]:
        if not self.god_core:
            return {"success": False, "error": "EMV God core unavailable"}
        self.god_core.set_task(task)
        return {"success": True, "active_task": task}

    def _tool_god_lesson(self, lesson: str) -> Dict[str, Any]:
        if not self.god_core:
            return {"success": False, "error": "EMV God core unavailable"}
        self.god_core.add_lesson(lesson)
        return {"success": True, "lesson_recorded": lesson}
    ...
    def _tool_simulate_apdu_mutation(
        ...
        if "gpo" in m_type or m_type == "all":
            ...
            forge_fn = getattr(mutations, "forge_second_gac_tc", None)
            if not callable(forge_fn):
                available = sorted(
                    name for name in dir(mutations)
                    if "gac" in name.lower() or "arpc" in name.lower()
                )
                raise AttributeError(
                    "mutations.forge_second_gac_tc is unavailable. "
                    f"Available GAC/ARPC helpers: {available}"
                )
            forged = forge_fn(cache, brand=constants.BRAND_UNKNOWN)
            res["mutated_hex"] = forged.hex().upper()
            res["changed"] = True
            res["explanation"] = "Synthesized second GENERATE AC TC (0x40) response from cached ARQC parameters."
        elif "arpc" in m_type or "rewrite" in m_type:
            # ARPC rewrite
            original_arpc = bytes.fromhex("00A4040000")
            rewrite_fn = getattr(mutations, "rewrite_arpc_response", None)
            if not callable(rewrite_fn):
                available = sorted(
                    name for name in dir(mutations)
                    if "arpc" in name.lower() or "rewrite" in name.lower()
                )
                raise AttributeError(
                    "mutations.rewrite_arpc_response is unavailable. "
                    f"Available ARPC/rewrite helpers: {available}"
                )
            rewritten, changed = rewrite_fn(
                original_arpc,
                brand=constants.BRAND_UNKNOWN,
            )
            res["mutated_hex"] = rewritten.hex().upper()
            res["changed"] = changed
            res["explanation"] = f"ARPC response rewritten. Changed={changed}"
        else:
    ...
    def _tool_modify_project_code(
        ...
        # --------------------------------------------------------
        # EMV God: verify the exact file now on disk.
        # --------------------------------------------------------

        god_verification = None
        god_core = getattr(self, "god_core", None)
        if god_core and hasattr(god_core, "verify_file"):
            try:
                god_verification = god_core.verify_file(str(resolved_path))
            except Exception:
                logger.exception("EMV God post-write verification failed")
    ...
    def _register_default_tools(self) -> None:
        ...
        self.register(AITool(
            name="ai_runtime_health",
            description="Return a non-blocking health snapshot of the local AI/tool infrastructure.",
            parameters={"type": "object", "properties": {}, "required": []},
            handler=lambda: self.runtime_health(),
            category="system",
        ))
        ...
        self.register(AITool(
            name="god_project_map",
            description="Inspect or refresh the living project map: Python files, symbols, hashes, active task, recent events, and learned lessons. Use before complex code changes.",
            parameters={"type":"object","properties":{"refresh":{"type":"boolean","default":False}},"required":[]},
            handler=self._tool_god_project_map, category="god_cognition",
        ))

        self.register(AITool(
            name="god_verify_change",
            description="Re-read a file after a mutation and verify its current SHA-256, size, line count, and Python syntax. Use after every code change.",
            parameters={"type":"object","properties":{"file_path":{"type":"string"},"expected_sha256":{"type":"string","default":""}},"required":["file_path"]},
            handler=self._tool_god_verify, category="god_verification",
        ))

        self.register(AITool(
            name="god_set_task",
            description="Set the persistent active engineering task in EMV God state.",
            parameters={"type":"object","properties":{"task":{"type":"string"}},"required":["task"]},
            handler=self._tool_god_set_task, category="god_memory",
        ))

        self.register(AITool(
            name="god_record_lesson",
            description="Persist a concise engineering lesson learned from a failure or successful verification.",
            parameters={"type":"object","properties":{"lesson":{"type":"string"}},"required":["lesson"]},
            handler=self._tool_god_lesson, category="god_memory",
        ))
