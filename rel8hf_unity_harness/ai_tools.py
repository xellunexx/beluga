...
class AIToolRegistry:
    ...
    def _tool_gui_controls(self, include_disabled: bool = True) -> Dict[str, Any]:
        """Inspect the currently attached Qt main window and return semantic controls."""
        app_rt = None
        window = None
        if self.app_runtime_getter:
            try:
                app_rt = self.app_runtime_getter()
                window = getattr(app_rt, "main_window", None) if app_rt else None
            except Exception as exc:
                return {"error": f"Unable to resolve active Qt main window: {exc}"}

        if window is None:
            return {
                "available": False,
                "error": "No active Qt main window is attached to AIToolRegistry.",
                "hint": "Use headless_gui with operation=inventory when the live window is unavailable.",
            }

        controls: List[Dict[str, Any]] = []
        seen = set()
        try:
            from qt_app import QWidget
        except Exception as exc:
            return {"available": False, "error": f"Unable to import Qt widget type: {exc}"}

        def add_control(obj: Any) -> None:
            ident = id(obj)
            if ident in seen:
                return
            seen.add(ident)
            ...
            try:
                ...
                pass
            try:
                text = str(obj.text() or "") if hasattr(obj, "text") else ""
            except Exception:
                pass
        ...
        # QApplication/Qt object graphs expose descendants through findChildren.
        try:
            for obj in window.findChildren(QWidget):
                if hasattr(obj, "click") and callable(getattr(obj, "click", None)):
                    add_control(obj)
        except Exception as exc:
            return {"available": False, "error": f"Qt control enumeration failed: {exc}"}
    ...
    def _tool_gui_click(self, target: str) -> Dict[str, Any]:
        """Click an enabled button/control in the currently attached Qt application."""
        target = (target or "").strip()
        if not target:
            raise ValueError("target must be a non-empty object name, text, or exact target string")

        if not self.app_runtime_getter:
            return {"success": False, "error": "Active Qt application runtime is not attached."}

        try:
            app_rt = self.app_runtime_getter()
            window = getattr(app_rt, "main_window", None) if app_rt else None
        except Exception as exc:
            return {"success": False, "error": f"Unable to resolve active Qt main window: {exc}"}

        if window is None:
            return {"success": False, "error": "No active Qt main window is attached."}

        matches = []
        try:
            from qt_app import QWidget, QApplication
            for obj in window.findChildren(QWidget):
                if not hasattr(obj, "click") or not callable(getattr(obj, "click", None)):
                    continue
                ...
                if target == object_name or target == text:
                    matches.append(obj)

            if not matches:
                return {
                    "success": False,
                    "error": f"No clickable Qt control matched target: {target}",
                    "hint": "Call gui_controls first to discover semantic object names and labels.",
                }

            if len(matches) > 1:
                return {
                    "success": False,
                    "error": f"Ambiguous GUI target '{target}': {len(matches)} matches.",
                    "hint": "Use a unique object_name or call gui_controls to disambiguate.",
                }
            ...
            if hasattr(obj, "isEnabled") and not obj.isEnabled():
                return {
                    "success": False,
                    "error": f"GUI control '{target}' is disabled.",
                }

            obj.click()

            # Give queued Qt signals/slots a chance to execute without requiring a visible UI.
            try:
            ...
            return {"success": False, "error": f"GUI click failed: {type(exc).__name__}: {exc}"}

    def _tool_headless_gui(
        self,
        operation: str = "inventory",
        target: str = "",
    ) -> Dict[str, Any]:
        """Run the project's offscreen Qt control plane (headless_gui/cli.py)."""
        operation = (operation or "inventory").strip().lower()
        allowed = {"audit", "inventory", "probe", "lifecycle", "click"}
        if operation not in allowed:
            raise ValueError(f"operation must be one of: {', '.join(sorted(allowed))}")
        if operation == "click" and not target.strip():
            raise ValueError("target is required when operation='click'")

        cli_path = Path(__file__).resolve().parent / "headless_gui" / "cli.py"
        if not cli_path.exists():
            return {
                "success": False,
                "error": f"Headless GUI CLI not found: {cli_path}",
            }
        ...
        try:
            ...
            return {"success": False, "error": "Headless GUI operation timed out after 120 seconds."}
        except Exception as exc:
            return {"success": False, "error": f"Failed to start headless GUI: {type(exc).__name__}: {exc}"}

        stdout = proc.stdout or ""
        stderr = proc.stderr or ""

        # cli.py writes JSON after possible Qt/logging lines. Recover the final JSON object/array.
        ...
        for idx in range(len(stdout) - 1, -1, -1):
            if stdout[idx] not in "[{":
                continue
            try:
                candidate, end = decoder.raw_decode(stdout[idx:])
                if not stdout[idx + end:].strip():
                    parsed = candidate
                    break
            except Exception:
                continue
        ...
        return {
            ...
            "stdout_tail": stdout[-12000:] if parsed is None else "",
    ...
    def _register_default_tools(self) -> None:
        ...
        self.register(AITool(
            name="gui_controls",
            description=(
                "Inspect the currently attached REL8 Qt application and enumerate its semantic clickable controls. "
                "Returns object names, visible text, classes, and enabled state. Use this before gui_click when the target is ambiguous."
            ),
            parameters={
                "type": "object",
                "properties": {
                    "include_disabled": {
                        "type": "boolean",
                        "default": True,
                        "description": "Include disabled controls in the returned inventory.",
                    },
                },
                "required": [],
            },
            handler=self._tool_gui_controls,
            category="gui",
        ))
        ...
        self.register(AITool(
            name="gui_click",
            description=(
                "Click an enabled control in the LIVE REL8 Qt application by exact object_name or visible text. "
                "This is semantic GUI control, not pixel/mouse automation. Call gui_controls first when needed."
            ),
            parameters={
                "type": "object",
                "properties": {
                    "target": {
                        "type": "string",
                        "description": "Exact Qt objectName or visible control text to click.",
                    },
                },
                "required": ["target"],
            },
            handler=self._tool_gui_click,
            category="gui",
        ))
        ...
        self.register(AITool(
            name="headless_gui",
            description=(
                "Operate the REL8 GUI through the project's OFFSCREEN headless Qt control plane. "
                "It can audit/inventory/probe/lifecycle the real Rel8AppletWindow and click semantic controls "
                "without a visible desktop. This runs the headless_gui/cli.py process and returns machine-readable results."
            ),
            parameters={
                "type": "object",
                "properties": {
                    "operation": {
                        "type": "string",
                        "enum": ["audit", "inventory", "probe", "lifecycle", "click"],
                        "default": "inventory",
                        "description": "Headless GUI operation to perform.",
                    },
                    "target": {
                        "type": "string",
                        "default": "",
                        "description": "Exact objectName or visible text for operation='click'.",
                    },
                },
                "required": [],
            },
            handler=self._tool_headless_gui,
            category="gui_headless",
        ))
...
def detect_and_execute_tools(user_query: str, registry: AIToolRegistry) -> List[ToolResult]:
    ...
    # (a0) GUI / headless control intent
    if any(k in q_lower for k in [
        "headless gui", "headless qt", "headless inventory", "headless audit",
        "headless probe", "headless lifecycle",
    ]):
        operation = "inventory"
        if "audit" in q_lower:
            operation = "audit"
        elif "probe" in q_lower:
            operation = "probe"
        elif "lifecycle" in q_lower:
            operation = "lifecycle"
        results.append(registry.execute_tool("headless_gui", operation=operation))

    if any(k in q_lower for k in [
        "click button", "click the button", "press button", "press the button",
        "click control", "press control",
    ]):
        quoted = re.findall(r"['\"]([^'\"]+)['\"]", q_str)
        target = quoted[0] if quoted else ""
        if target:
            results.append(registry.execute_tool("gui_click", target=target))
