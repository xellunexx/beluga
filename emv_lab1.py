...
class LabCardProfile:
    ...
    ext_auth_capdu: bytes = bytes.fromhex(
        "008200000A11223344556677883030"
    )
...
class LabTag(_Peer):
    ...
    def _fixture_for_apdu(
            self,
            apdu: bytes,
    ) -> Optional[Tuple[bytes, str]]:
        """
        Return a response only when the received CAPDU has an explicit
        byte-for-byte fixture.

        The tag is deliberately data-only:
          - no EMV state machine
          - no inferred next command
          - no synthetic GENERATE AC handling
          - no automatic cryptogram generation
          - no fallback based on INS/P1/P2

        This makes the lab an exact card-response oracle. Anything not
        explicitly captured/stored is a MISS and must be handled by the
        caller as an unknown CAPDU.
        """
        raw_apdu = bytes(apdu)

        fixture = self.response_fixtures.get(raw_apdu)
        if fixture is not None:
            return fixture

        if len(raw_apdu) > 5 and raw_apdu[-1] == 0x00:
            stripped = raw_apdu[:-1]
            fixture = self.response_fixtures.get(stripped)
            if fixture is not None:
                return fixture

        if len(raw_apdu) > 4:
            padded = raw_apdu + b"\x00"
            fixture = self.response_fixtures.get(padded)
            if fixture is not None:
                return fixture
...
class LabReader(_Peer):
    ...
    def __init__(self, profile: LabCardProfile, *, cdcvm: bool = False, **kwargs: Any) -> None:
        super().__init__(
            logical_role="READER",
            relay_role="EMULATOR",
            **kwargs,
        ...
        if self.cdcvm:
            gac = bytearray(profile.gac1_capdu)
            if len(gac) >= 5 + 43:
                gac[5 + 40] = 0x3F
                gac[5 + 41] = 0x00
                gac[5 + 42] = 0x00
                profile.gac1_capdu = bytes(gac)
...
def main() -> int:
    ...
    parser.add_argument("--cdcvm", action="store_true",
